...
final class AggregateCacheTest extends TestCase
{
    ...
    */
    ...
    {
        $this->doTestMemcached(CyclicAggregateCache::create()->setSummaryWeight(42)
    ...
    */
    public function testIntegerChanges()
    {
        ...
        for ($i = 0; $i < self::QUERIES; ++$i) {
            $this->assertEquals($i + 2, Cache::me()->increment('test_integer', 1));
            $this->assertEquals($i + 2, Cache::me()->get('test_integer'));
        }
        $this->assertEquals(self::QUERIES + 1, Cache::me()->get('test_integer'));
        for ($i = 0; $i < self::QUERIES; ++$i) {
            $this->assertEquals(self::QUERIES - $i, Cache::me()->decrement('test_integer', 1));
            $this->assertEquals(self::QUERIES - $i, Cache::me()->get('test_integer'));
        }
