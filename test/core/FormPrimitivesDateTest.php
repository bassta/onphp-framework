...
final class FormPrimitivesDateTest extends TestCase
{
    ...
    {
        $data  = array(PrimitiveDate::DAY => self::VALID_DAY, PrimitiveDate::MONTH => self::VALID_MONTH, PrimitiveDate::YEAR => self::VALID_YEAR);
        $scope = array('test' => $data);
        $this->processValidScopeBy($scope, $data);
        $data  = self::VALID_YEAR.'-'.self::VALID_MONTH.'-'.self::VALID_DAY;
        $scope = array('test' => $data);
        $this->processValidScopeBy($scope, $data);
    }
    ...
    {
        $data  = array(PrimitiveDate::DAY => self::INVALID_DAY, PrimitiveDate::MONTH => self::INVALID_MONTH, PrimitiveDate::YEAR => self::INVALID_YEAR);
        $scope = array('test' => $data);
        $this->processInvalidBy($scope, $data);
        $data  = self::INVALID_YEAR.'-'.self::INVALID_MONTH.'-'.self::INVALID_DAY;
        $scope = array('test' => $data);
        $this->processInvalidBy($scope, $data);
    }
    ...
    {
        $form = Form::create()->add(Primitive::date('test'))->import($scope);
        $this->assertEquals($form->getValue('test')->getDay(), (int)self::VALID_DAY);
        $this->assertEquals($form->getValue('test')->getMonth(), (int)self::VALID_MONTH);
        $this->assertEquals($form->getValue('test')->getYear(), (int)self::VALID_YEAR);
    ...
    protected function processInvalidBy($scope, $data)
    {
        ...
        $this->assertEquals($form->getRawValue('test'), $data);
        $this->assertEquals($form->get('test')->isImported(), true);
        $this->assertEquals($form->getErrors(), array('test' => Form::WRONG));
    }
