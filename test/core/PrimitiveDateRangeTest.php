...
final class PrimitiveDateRangeTest extends TestCase
{
    public function testImport()
    {
        ...
        $this->assertTrue($prm->getValue()->getStart()->getYear() == 1985);
        $this->assertEquals($prm->getValue()->toDateString(), $array['test']);
        $array = array('test' => $start);
        $this->assertTrue($prm->import($array));
        $this->assertEquals($prm->getValue()->toDateString(), $start.$delimiter.$start);
        $array = array('test' => $start.$delimiter);
        $this->assertTrue($prm->import($array));
        $this->assertEquals($prm->getValue()->toDateString(), $start);
        $array = array('test' => $delimiter.$start);
        $this->assertTrue($prm->import($array));
        $this->assertEquals($prm->getValue()->toDateString(), $start);
        $array = array('test' => $delimiter);
        $this->assertTrue($prm->import($array));
        $this->assertEquals($prm->getValue()->toDateString(), '');
        $array = array('test' => '---');
    ...
    public function testDottedImport()
    {
        ...
        $this->assertTrue($prm->import($array));
        $this->assertEquals($prm->getValue()->toDateString(), $result.$delimiter.$result);
        $date   = '02';
        $array  = array('test' => $date);
        $now    = Date::makeToday();
    ...
    public function testUndelimitedDate()
    {
        ...
        $result = $now->getYear().'-02-14';
        $this->assertTrue($prm->import($array));
        $this->assertEquals($prm->getValue()->toDateString(), $result.' - '.$result);
    }
}