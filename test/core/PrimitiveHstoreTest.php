...
{
    protected static $scope = array('properties' => array('age' => '23', 'weight' => 80, 'comment' => 'test user case'));
    protected static $invalidScope = array('properties' => array('weight' => 'test error'));

    public function testImport()
    {
        $prm = $this->create();
        $this->assertTrue($prm->import(self::$scope));
        ...
        $this->assertEquals($hstore->getList(), self::$scope['properties']);
        $this->assertEquals($prm->exportValue(), self::$scope['properties']);
        try {
            $hstore->get('NotFound');
            $this->fail('NotFound');
    ...
    {
        $prm = $this->create();
        $this->assertFalse($prm->import(self::$invalidScope));
        $subform = $prm->getInnerForm();
        $this->assertNull($subform->getValue('weight'));
        $this->assertEquals($prm->getInnerErrors(), array('weight' => Form::WRONG));
