...
class InnerTransactionTest extends TestCase
{
    ...
    {
        //setup
        $db = $this->spawnDb(array('begin' => 1, 'commit' => 1));
        //execute
    ...
    {
        //setup
        $db = $this->spawnDb(array('savepointBegin' => 1, 'savepointRelease' => 1, 'inTransaction' => true));
        //execute
        $transaction = InnerTransaction::begin($db);
        $transaction->commit();
        //test Exception on second commit
    ...
    public function testWrapCommit()
    {
        ...
        $wrapper       = InnerTransactionWrapper::create()->setDB($db)->setFunction($innerFunction);
    ...
    public function testWrapRollbackByException()
    {
        ...
        $wrapper            = InnerTransactionWrapper::create()->setDB($db)
    ...
    {
        $options += array('begin' => 0, 'commit' => 0, 'rollback' => 0, 'savepointBegin' => 0, 'savepointRelease' => 0, 'savepointRollback' => 0, 'inTransaction' => false);
        $mock         = $this->getMock('PgSQL');
        $countMethods = array('begin', 'commit', 'rollback', 'savepointBegin', 'savepointRelease', 'savepointRollback');
        foreach ($countMethods as $method) {
            $mock->expects($this->exactly($options[$method]))->method($method)->will($this->returnSelf());
        }
