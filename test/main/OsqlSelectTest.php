...
final class OsqlSelectTest extends TestCaseDB
{
    public function testSelectGet()
    {
        ...
        $query   = OSQL::select()->from('test_table')->get(DBField::create('field1', 'test_table'), 'alias1')
                       ->get(DBField::create('field2', 'test_table'))->get('field3', 'alias3')->get('field4')
                       ->get(SQLFunction::create('count', DBField::create('field5', 'test_table'))
                                        ->setAggregateDistinct()->setAlias('alias5'))
                       ->get(SQLFunction::create('substring', BinaryExpression::create(DBField::create('field6', 'test_table'), DBValue::create('a..b'), 'from')
                                                                              ->noBrackets()));
    ...
    {
        $dialect = $this->getDbByType('PgSQL')->getDialect();
        $query   = OSQL::select()->from('test_table')->get('field1')->get(OSQL::select()->from('test_table1')
                                                                              ->setName('foo1')->get('id'));
    ...
    {
        return OSQL::select()->from('table1')->get(DBField::create('field1', 'table1'))
