...
final class OsqlReturningTest extends TestCaseDB
{
    public function testUpdate()
    {
        ...
        $query   = OSQL::update('test_table')->set('field1', 1)->where(Expression::eq('field1', 2));
        $this->addReturning($query);
        $this->assertEquals($query->toDialectString($dialect), 'UPDATE "test_table" '.'SET "field1" = \'1\' '.'WHERE ("field1" = \'2\') '.'RETURNING '.'"test_table"."field1" AS "alias1", '.'"test_table"."field2", '.'count("test_table"."field5") AS "alias5", '.'(SELECT "test_table1"."id" FROM "test_table1") AS "foo1"');
    }
    ...
    {
        $query->returning(DBField::create('field1', 'test_table'), 'alias1')->returning('field2')
              ->returning(SQLFunction::create('count', DBField::create('field5', 'test_table'))->setAlias('alias5'))
              ->returning(OSQL::select()->from('test_table1')->setName('foo1')->get('id'));
        return $query;
    }
}