...
class AMQPTestCaseNoAckQueueConsumer extends AMQPPeclQueueConsumer
{
    ...
    public function handleDelivery(AMQPIncomingMessage $delivery)
    {
        ...
        $this->getChannel()->basicAck($delivery->getDeliveryTag(), true);
...
class AMQPPeclTest extends TestCase
{
     ...
     */
    const PORT_MIRRORED = 5673;
    // port of slave node
    protected static $queueList = array('basic' => array('exchange' => 'AMQPPeclTestExchange', 'exchangeType' => AMQPExchangeType::DIRECT, 'name' => 'AMQPPeclTestQueue', 'key' => 'routing.key', 'args' => array()), 'exchangeBinded' => array('exchange' => 'AMQPPeclTestExchangeBinded', 'exchangeType' => AMQPExchangeType::FANOUT, 'name' => 'AMQPPeclTestQueueBinded', 'key' => 'routing.key.binded', 'args' => array()), 'mirrored' => array('exchange' => 'AMQPPeclTestExchange', 'exchangeType' => AMQPExchangeType::DIRECT, 'name' => 'AMQPPeclTestQueueMirrored', 'key' => 'routing.key.mirrored', 'args' => array('x-ha-policy' => 'all')));
     ...
     * @param int $value
     */
    public static function checkMessageCount(AMQPChannelInterface $channel, $label = 'basic', $value = self::COUNT_OF_PUBLISH)
    {
        usleep(self::MESSAGE_COUNT_WAIT);
        self::assertTrue(isset(self::$queueList[$label]));
        $count = $channel->queueDeclare(self::$queueList[$label]['name'], AMQPQueueConfig::create()->setDurable(true)
                                                                                         ->setArguments(self::$queueList[$label]['args']));
    ...
    {
        try {
            $c = new AMQPPecl(AMQPCredentials::create()->setHost('localhost')->setPort(5672)->setLogin('guest')
    ...
    public function testNoAckConsumerLogic()
    {
        ...
        for ($j = 1; $j <= self::COUNT_OF_PUBLISH; $j++) {
            $mess = $channel->basicGet(self::$queueList['basic']['name'], false);
    ...
    {
        $c = AMQPSelective::me()->addLink('slave', new AMQPPecl(AMQPCredentials::createDefault()
                                                                               ->setPort(self::PORT_MIRRORED)))
                          ->addLink('master', new AMQPPecl(AMQPCredentials::createDefault()))->setCurrent('slave');
        $c->dropLink('slave');
        $channel = $c->createChannel(1);
    ...
    public function testProducerLogicMirrored()
    {
        $c       = AMQPSelective::me()->addLink('slave', new AMQPPecl(AMQPCredentials::createDefault()
                                                                                     ->setPort(self::PORT_MIRRORED)))
                                ->addLink('master', new AMQPPecl(AMQPCredentials::createDefault()))
                                ->setCurrent('slave');
        $channel = $c->createChannel(1);
        ...
        AMQPPeclTest::assertEquals(self::PORT_MIRRORED, $c->getCredentials()->getPort());
        $c->dropLink('slave');
    ...
    public function testQueueConsumerNoAck()
    {
        ...
        $channel->basicConsume(self::$queueList['basic']['name'], false, $consumer);
    ...
    public function testQueueConsumerAutoAck()
    {
        ...
        $consumer->setLimit(AMQPPeclTest::COUNT_OF_PUBLISH);
        $channel->basicConsume(self::$queueList['basic']['name'], true, $consumer);
        //observer logic test
        $this->assertSame('AC', $consumer->getCheckString());
        $this->assertEquals(self::COUNT_OF_PUBLISH, $consumer->getCount());
        //drop channels and close connection
    ...
    public function testExchangeToExchangeProducerLogic()
    {
         ...
         */
        $channelInterface = $channel->exchangeBind(self::$queueList['basic']['exchange'], self::$queueList['exchangeBinded']['exchange'], self::$queueList['basic']['key']);
        ...
        for ($i = 1; $i <= self::COUNT_OF_PUBLISH; $i++) {
            $channelInterface = $channel->basicPublish(self::$queueList['exchangeBinded']['exchange'], self::$queueList['basic']['key'], AMQPOutgoingMessage::create()
                                                                                                                                                            ->setBody("message {$i}")
    ...
    public function testExchangeToExchangeConsumerLogic()
    {
        $c       = new AMQPPecl(AMQPCredentials::createDefault());
        $channel = $c->createChannel(1);
        $names   = array(self::$queueList['basic']['name'], self::$queueList['exchangeBinded']['name']);
        foreach ($names as $name) {
            ...
            $this->assertSame(self::COUNT_OF_PUBLISH, $i, "message count={$i} in {$name}-queue, must be equal to ".self::COUNT_OF_PUBLISH);
        }
    }
    ...
    protected function exchangeToExchangeCleanup()
    {
        ...
        $this->queueBind($channel, 'basic');
        $this->queueBind($channel, 'exchangeBinded');
        $channel->queueBind(self::$queueList['basic']['name'], self::$queueList['exchangeBinded']['exchange'], self::$queueList['basic']['key']);
        $this->queueUnbind($channel, 'basic');
        $this->queueUnbind($channel, 'exchangeBinded');
        $channelInterface = $channel->queueUnbind(self::$queueList['basic']['name'], self::$queueList['exchangeBinded']['exchange'], self::$queueList['basic']['key']);
        $this->assertInstanceOf('AMQPChannelInterface', $channelInterface);
    ...
    {
        for ($i = 1; $i <= self::COUNT_OF_PUBLISH; $i++) {
            $channelInterface = $channel->basicPublish(self::$queueList[$label]['exchange'], self::$queueList[$label]['key'], AMQPOutgoingMessage::create()
                                                                                                                                                 ->setBody("message {$i}")
                                                                                                                                                 ->setTimestamp(Timestamp::makeNow())
                                                                                                                                                 ->setAppId(__CLASS__)
                                                                                                                                                 ->setMessageId($i)
    ...
    protected static function messageAssertion(AMQPIncomingMessage $mess, $i)
    {
        ...
        self::assertTrue(isset($properties[AMQPIncomingMessage::USER_ID]) && $properties[AMQPIncomingMessage::USER_ID] == $mess->getUserId());
        self::assertEquals(__CLASS__, $mess->getAppId());
        self::assertTrue(isset($properties[AMQPIncomingMessage::APP_ID]) && $properties[AMQPIncomingMessage::APP_ID] == $mess->getAppId());
        self::assertEquals($i, $mess->getMessageId());
        self::assertTrue(isset($properties[AMQPIncomingMessage::MESSAGE_ID]) && $properties[AMQPIncomingMessage::MESSAGE_ID] == $mess->getMessageId());
        self::assertEquals('text/plain', $mess->getContentType());
        self::assertTrue(isset($properties[AMQPIncomingMessage::CONTENT_TYPE]) && $properties[AMQPIncomingMessage::CONTENT_TYPE] == $mess->getContentType());
        self::assertEquals('utf-8', $mess->getContentEncoding());
        self::assertTrue(isset($properties[AMQPIncomingMessage::CONTENT_ENCODING]) && $properties[AMQPIncomingMessage::CONTENT_ENCODING] == $mess->getContentEncoding());
    ...
    protected function exchangeDeclare(AMQPChannelInterface $channel, $label)
    {
        ...
        $interface = $channel->exchangeDeclare(self::$queueList[$label]['exchange'], AMQPExchangeConfig::create()
                                                                                                       ->setType(new AMQPExchangeType(self::$queueList[$label]['exchangeType']))
                                                                                                       ->setDurable(true));
    ...
    {
        $this->assertTrue(isset(self::$queueList[$label]));
        return $channel->queueDeclare(self::$queueList[$label]['name'], AMQPQueueConfig::create()->setDurable(true)
                                                                                       ->setArguments(self::$queueList[$label]['args']));
    }
    ...
    {
        $this->assertTrue(isset(self::$queueList[$label]));
        $channelInterface = $channel->queueUnbind(self::$queueList[$label]['name'], self::$queueList[$label]['exchange'], self::$queueList[$label]['key']);
        $this->assertInstanceOf('AMQPChannelInterface', $channelInterface);
        return $channelInterface;
    }
