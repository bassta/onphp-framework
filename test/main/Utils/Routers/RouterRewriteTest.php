...
class RouterRewriteTest extends TestCase
{
    ...
    {
        $this->router = RouterRewrite::me()->resetAll()->setBaseUrl(HttpUrl::create());
    ...
    {
        $this->router->addRoute('archive', RouterTransparentRule::create('archive/:year')
                                                                ->setDefaults(array('year' => '2006', 'area' => 'archive', 'action' => 'show'))
                                                                ->setRequirements(array('year' => '\\d+')));
        $routes = $this->router->getRoutes();
        $this->assertEquals(1, count($routes));
        $this->assertInstanceOf('RouterTransparentRule', $routes['archive']);
        $this->router->addRoute('register', RouterTransparentRule::create('register/:action')
                                                                 ->setDefaults(array('area' => 'profile', 'action' => 'register')));
        $routes = $this->router->getRoutes();
    ...
    {
        $routes = array('archive' => RouterTransparentRule::create('archive/:year')
                                                          ->setDefaults(array('year' => '2006', 'area' => 'archive', 'action' => 'show'))
                                                          ->setRequirements(array('year' => '\\d+')), 'register' => RouterTransparentRule::create('register/:action')
                                                                                                                                         ->setDefaults(array('area' => 'profile', 'action' => 'register')));
    ...
    {
        $archive = RouterTransparentRule::create('archive/:year')
                                        ->setDefaults(array('year' => '2006', 'area' => 'archive', 'action' => 'show'))
                                        ->setRequirements(array('year' => '\\d+'));
    ...
    public function testEmptyRoute()
    {
        ...
        $this->router->addRoute('empty', RouterTransparentRule::create('')
                                                              ->setDefaults(array('area' => 'ctrl', 'action' => 'act')));
        $token = $this->router->route($request);
    ...
    {
        $request = $this->buildRequest('http://localhost/');
        $this->router->addRoute('catch-all', RouterTransparentRule::create(':area/:action/*')
                                                                  ->setDefaults(array('area' => 'ctrl', 'action' => 'act')));
        $token = $this->router->route($request);
        $this->assertEquals('ctrl', $token->getAttachedVar('area'));
        $this->assertEquals('act', $token->getAttachedVar('action'));
    ...
    {
        $request = $this->buildRequest('http://localhost/');
        $this->router->addRoute('catch-all', RouterTransparentRule::create('*')
                                                                  ->setDefaults(array('area' => 'ctrl', 'action' => 'act')));
        $token = $this->router->route($request);
        $this->assertEquals('ctrl', $token->getAttachedVar('area'));
        $this->assertEquals('act', $token->getAttachedVar('action'));
    ...
    {
        $request = $this->buildRequest('http://localhost/archive/action/bogus');
        $this->router->addRoute('default', new RouterTransparentRule(':area/:action'));
        $token = $this->router->route($request);
    ...
    public function testFirstRouteMatched()
    {
        ...
        try {
            $this->assertEquals('archive', $this->router->getCurrentRouteName());
            $this->assertInstanceOf('RouterTransparentRule', $this->router->getCurrentRoute());
    ...
    {
        $request = $this->buildRequest('http://localhost/archive/2006/param/07');
        $this->router->addRoute('test', new RouterTransparentRule(':area/:action/*'));
        $token = $this->router->route($request);
        $this->assertEquals('archive', $token->getAttachedVar('area'));
    ...
    {
        $this->router->addRoute('foo', RouterTransparentRule::create(':lang/foo')
                                                            ->setDefaults(array('lang' => 'nl', 'area' => 'index', 'action' => 'index')));
        $this->router->addRoute('bar', RouterTransparentRule::create(':lang/bar')
                                                            ->setDefaults(array('lang' => 'nl', 'area' => 'index', 'action' => 'index')));
        $request = $this->buildRequest('http://localhost/nl/bar');
        $token   = $this->router->route($request);
        $this->assertEquals('nl/foo', $this->router->getRoute('foo')->assembly());
        $this->assertEquals('nl/bar', $this->router->getRoute('bar')->assembly());
    }
    ...
    public function testUrlValuesHandling2()
    {
        ...
        $token   = $this->router->route($request);
        $this->assertEquals('en/foo', $this->router->getRoute('foo')->assembly());
        $this->assertEquals('nl/bar', $this->router->getRoute('bar')->assembly());
    }
    ...
    public function testUrlValuesHandling3()
    {
        ...
        $token   = $this->router->route($request);
        $this->assertEquals('nl/foo', $this->router->getRoute('foo')->assembly());
        $this->assertEquals('en/bar', $this->router->getRoute('bar')->assembly());
    }
    ...
    public function testRoutingChainedRoutes()
    {
        ...
        $foo     = RouterTransparentRule::create('foo')->setDefaults(array('foo' => true));
        $bar     = RouterTransparentRule::create('bar')
                                        ->setDefaults(array('bar' => true, 'area' => 'foo', 'action' => 'bar'));
    ...
    public function testRouteWithHostnameChain()
    {
        ...
        $foo        = RouterHostnameRule::create('nope.example.com')
                                        ->setDefaults(array('module' => 'nope-bla', 'bogus' => 'bogus'));
    ...
    public function testRouteWithHostnameAndTransparentRuleChain()
    {
        ...
        $host        = RouterHostnameRule::create('www.example.com')
                                         ->setDefaults(array('module' => 'nope-bla', 'bogus' => 'bogus'));
        $transparent = RouterTransparentRule::create(':area/:contest')
                                            ->setRequirements(array('contest' => '\\d+', 'area' => '\\w+'));
        $chain       = new RouterChainRule();
    ...
    public function testRouteWithHostnameAndTransparentAndBaseUrlRuleChain()
    {
        ...
        $host        = RouterHostnameRule::create('www.example.com')
                                         ->setDefaults(array('module' => 'nope-bla', 'domain' => 'www'));
        $transparent = RouterTransparentRule::create(':area/:contest')
                                            ->setRequirements(array('contest' => '\\d+', 'area' => '\\w+'));
        $chain       = new RouterChainRule();
        $chain->chain($host)->chain($transparent);
    ...
    public function testRouteWithHostnameMaskAndRegexpAndBaseUrlRuleChain()
    {
        ...
        $host        = RouterHostnameRule::create(':subdomain.example.com')
                                         ->setDefaults(array('module' => 'nope-bla', 'area' => 'test'))
                                         ->setRequirements(array('subdomain' => '[\\da-z][\\da-z\\_]*[\\da-z]'));
        $transparent = RouterRegexpRule::create('test/(\\d+)\\.html')->setDefaults(array(1 => 345))
                                       ->setMap(array(1 => 'testId'));
        $chain       = new RouterChainRule();
        $chain->chain($host)->chain($transparent);
        $this->router->setBaseUrl(HttpUrl::create()->parse($base));
    ...
    {
        $this->router->addRoute('contest', RouterTransparentRule::create('contest/:contest/*')
                                                                ->setDefaults(array('area' => 'contestController'))
                                                                ->setRequirements(array('contest' => '\\d+')));
        $this->assertEquals('/contest/123', $this->router->assembly(array('area' => 'contestController', 'contest' => 123), 'contest'));
    }
    ...
    public function testAssemblingWithHostnameHttp()
    {
        ...
        $this->assertEquals('http://www.example.com', $this->router->assembly(array(), 'hostname-route'));
    }

    public function testAssemblingWithHostnameHttps()
    {
        $route = RouterHostnameRule::create('www.example.com')->setSecure();
        $this->router->addRoute('hostname-route', $route);
        $this->assertEquals('https://www.example.com', $this->router->assembly(array(), 'hostname-route'));
    }
    ...
    public function testAssemblingWithHostnameThroughChainHttp()
    {
        ...
        $chain = RouterChainRule::create()->chain($foo)->chain($bar);
        $this->router->addRoute('foo-bar', $chain);
    ...
    public function testAssemblingWithHostnameThroughChainHttpAndBaseUrl()
    {
        ...
        $bar   = new RouterStaticRule('bar');
        $chain = RouterChainRule::create()->chain($foo)->chain($bar);
        $this->router->setBaseUrl(HttpUrl::create()->parse('http://www.example.com/~user/public/'))
                     ->addRoute('foo-bar', $chain);
        $this->assertEquals('http://www.example.com/~user/public/bar', $this->router->assembly(array(), 'foo-bar'));
    }
    ...
    public function testAssemblingWithHostnameThroughChainHttpAndBaseUrlAndDiffHost()
    {
        ...
        $bar   = new RouterStaticRule('bar');
        $chain = RouterChainRule::create()->chain($foo)->chain($bar);
        $this->router->setBaseUrl(HttpUrl::create()->parse('http://qwerty.example.com/~user/public/'))
                     ->addRoute('foo-bar', $chain);
        $this->assertEquals('http://www.example.com/~user/public/bar', $this->router->assembly(array(), 'foo-bar'));
    }
    ...
    public function testAssemblingWithHostnameThroughChainHttpAndBaseUrlAndDiffScheme()
    {
        ...
        $bar   = new RouterStaticRule('bar');
        $chain = RouterChainRule::create()->chain($foo)->chain($bar);
        $this->router->setBaseUrl(HttpUrl::create()->parse('https://www.example.com/~user/public/'))
                     ->addRoute('foo-bar', $chain);
        $this->assertEquals('http://http.example.com/bar', $this->router->assembly(array(), 'foo-bar'));
    }

    public function testAssemblingWithSettingHostnameThroughChainHttpAndBaseUrlAndDiffScheme()
    {
        $foo   = RouterHostnameRule::create('https.example.com')->setSecure();
        $bar   = new RouterStaticRule('bar');
        $chain = RouterChainRule::create()->chain($foo)->chain($bar);
        $this->router->setBaseUrl(HttpUrl::create()->parse('https://www.example.com/~user/public/'))
                     ->addRoute('foo-bar', $chain);
        $this->assertEquals('https://https.example.com/~user/public/bar', $this->router->assembly(array(), 'foo-bar'));
    }
    ...
    {
        $route = RouterTransparentRule::create('blog/articles/:id')
                                      ->setDefaults(array('area' => 'blog', 'action' => 'articles', 'id' => 0))
                                      ->setRequirements(array('id' => '[0-9]+'));
    ...
    public function testAssemblingWithHostnameAndBaseUrl()
    {
        ...
        $hostname = RouterHostnameRule::create(':subdomain.example.com')->setDefaults(array('subdomain' => 'mega'));
        $this->router->setBaseUrl(HttpUrl::create()->parse($base));
        $this->router->addRoute('host', $hostname);
        $this->assertEquals('http://test.example.com/~users/public/www', $this->router->assembly(array('subdomain' => 'test'), 'host'));
    }
