...
class RouterChainRuleTest extends TestCase
{
    ...
    public function testChainingMatch()
    {
        ...
        $foo   = RouterHostnameRule::create('www.example.com')->setDefaults(array('foo' => 1));
        $bar   = RouterStaticRule::create('bar')->setDefaults(array('bar' => 2));
    ...
    {
        $foo     = RouterHostnameRule::create('www.example.com')->setDefaults(array('foo' => 1));
        $bar     = RouterStaticRule::create('bar')->setDefaults(array('bar' => 2, 'area' => 'foo', 'action' => 'bar'));
        $chain   = $foo->chain($bar);
        $request = $this->buildRequest('http://www.example.com/bar');
        $res     = $chain->match($request);
        $this->assertEquals(1, $res['foo']);
    ...
    {
        $foo     = RouterHostnameRule::create('www.example.com')
                                     ->setDefaults(array('foo' => 1, 'area' => 'foo', 'module' => 'foo'));
        $bar     = RouterTransparentRule::create('bar')
                                        ->setDefaults(array('bar' => 2, 'area' => 'bar', 'action' => 'bar'));
        $chain   = $foo->chain($bar);
        $request = $this->buildRequest('http://www.example.com/bar');
        $res     = $chain->match($request);
    ...
    public function testChainingSeparatorOverriding()
    {
        ...
        $foo   = RouterTransparentRule::create('foo')->setDefaults(array('foo' => 1));
        $bar   = RouterTransparentRule::create('bar')->setDefaults(array('bar' => 2));
        $baz   = RouterTransparentRule::create('baz')->setDefaults(array('baz' => 3));
        $chain = $foo->chain($bar, '.');
    ...
    {
        $this->markTestSkipped('Route features not ready');
        $lang    = RouterTransparentRule::create(':lang')->setDefaults(array('lang' => 'en'));
        $profile = RouterTransparentRule::create('user/:id')->setDefaults(array('area' => 'foo', 'action' => 'bar'));
    ...
    {
        $foo          = RouterHostnameRule::create('www.example.com')->setDefaults(array('foo' => 'foo'));
        $profile      = RouterTransparentRule::create('user/:id')->setDefaults(array('area' => 'prof'));
        $article      = RouterTransparentRule::create('article/:id')
                                             ->setDefaults(array('area' => 'art', 'action' => 'art'));
