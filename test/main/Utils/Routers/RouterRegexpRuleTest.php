...
class RouterRegexpRuleTest extends TestCase
{
    ...
    {
        $route  = RouterRegexpRule::create('users/all')->setDefaults(array('area' => 'ctrl'));
        $values = $route->match($this->buildRequest('http://localhost/users/all'));
    ...
    {
        $route  = RouterRegexpRule::create('((?!admin|moderator).+)')
                                  ->setDefaults(array('module' => 'index', 'area' => 'index'))
                                  ->setMap(array(1 => 'action'));
    ...
    {
        $route  = RouterRegexpRule::create('users/?(.+)?')->setDefaults(array(1 => 'martel'));
        $values = $route->match($this->buildRequest('http://localhost/users'));
        $this->assertSame(1, count($values));
        $this->assertSame('martel', $values[1]);
    }
    ...
    {
        $route  = RouterRegexpRule::create('users/(.+)')->setMap(array(1 => 'username'));
        $values = $route->match($this->buildRequest('http://localhost/users/martel'));
        $this->assertSame(1, count($values));
    ...
    {
        $route  = RouterRegexpRule::create('users(?:/(.+))?')->setDefaults(array('username' => 'martel'))
                                  ->setMap(array(1 => 'username'));
        $values = $route->match($this->buildRequest('http://localhost/users'));
        $this->assertSame(1, count($values));
        $this->assertSame('martel', $values['username']);
    }
    ...
    {
        $route  = RouterRegexpRule::create('users/(?P<name>.+)')->setMap(array(1 => 'username'));
        $values = $route->match($this->buildRequest('http://localhost/users/martel'));
        $this->assertSame(1, count($values));
        $this->assertSame('martel', $values['username']);
    }
    ...
    {
        $route  = RouterRegexpRule::create('users/(\\w+)/?(?:p/(\\d+))?')->setMap(array(1 => 'username', 2 => 'page'));
    ...
    {
        $route  = RouterRegexpRule::create('users/(\\w+)/?(?:p/(\\d+))?')->setMap(array(1 => 'username'));
        $values = $route->match($this->buildRequest('http://localhost/users/martel/p/1'));
        $this->assertSame(2, count($values));
        $this->assertSame('martel', $values['username']);
    ...
    {
        $route  = RouterRegexpRule::create('users/(\\w+)/?(?:p/(\\d+))?')->setDefaults(array(2 => '1'))
                                  ->setMap(array(1 => 'username'));
    ...
    {
        $route  = RouterRegexpRule::create('users/?(\\w+)?/?(?:p/(\\d+))?')
                                  ->setDefaults(array(2 => '1', 'username' => 'martel'))
                                  ->setMap(array(1 => 'username'));
        $values = $route->match($this->buildRequest('http://localhost/users'));
        $this->assertSame(2, count($values));
        $this->assertSame('martel', $values['username']);
    ...
    {
        $route  = RouterRegexpRule::create('users/(\\w+)/?(?:p/(\\d+))?')
                                  ->setDefaults(array('page' => '1', 'username' => 'martel'))
                                  ->setMap(array(1 => 'username', 2 => 'page'));
        $values = $route->match($this->buildRequest('http://localhost/users/martel'));
        $this->assertSame(2, count($values));
        $this->assertSame('martel', $values['username']);
        $this->assertSame('1', $values['page']);
    ...
    {
        $route  = RouterRegexpRule::create('users/?(\\w+)?')->setDefaults(array(1 => 'martel', 'username' => 'vicki'))
                                  ->setMap(array(1 => 'username'));
        $values = $route->match($this->buildRequest('http://localhost/users'));
        $this->assertSame(1, count($values));
    ...
    {
        $route  = RouterRegexpRule::create('users/(.+)')->setMap(array(1 => 'username'))->setReverse('users/%s');
        $values = $route->match($this->buildRequest('http://localhost/users/martel'));
        $url    = $route->assembly();
    ...
    {
        $route  = RouterRegexpRule::create('users/?(.+)?')->setDefaults(array(1 => 'martel'))->setReverse('users/%s');
        $values = $route->match($this->buildRequest('http://localhost/users'));
        $url    = $route->assembly();
        $this->assertSame('users/martel', $url);
    }
    ...
    {
        $route  = RouterRegexpRule::create('users/?(.+)?')->setDefaults(array('username' => 'martel'))
                                  ->setMap(array(1 => 'username'))->setReverse('users/%s');
        $values = $route->match($this->buildRequest('http://localhost/users'));
        $url    = $route->assembly();
        $this->assertSame('users/martel', $url);
    }
    ...
    {
        $route  = RouterRegexpRule::create('users/(.+)')->setReverse('users/%s');
        $values = $route->match($this->buildRequest('http://localhost/users/martel'));
    ...
    {
        $route = RouterRegexpRule::create('(.+)\\.([0-9]+)-([0-9]+)\\.html')
                                 ->setDefaults(array('module' => 'default', 'area' => 'content.item', 'action' => 'forward'))
                                 ->setMap(array(1 => 'name', 2 => 'id', 3 => 'class'))->setReverse('%s.%s-%s.html');
        ...
        $url = $route->assembly(array('name' => 'post_name', 'id' => '12', 'class' => 5));
        $this->assertSame('post_name.12-5.html', $url);
    }

    public function testCreateInstance()
    {
        $route = RouterRegexpRule::create('forum/(\\d+)')->setDefaults(array('area' => 'ctrl'))->setReverse('forum/%d');
    ...
    {
        $route = RouterRegexpRule::create('itemlist(?:/(\\d+))?')->setDefaults(array('page' => 1))
                                 ->setMap(array(1 => 'page'))->setReverse('itemlist/%d');
