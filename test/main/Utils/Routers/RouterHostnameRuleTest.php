...
class RouterHostnameRuleTest extends TestCase
{
    ...
    {
        $route = $this->buildStaticHostnameRule();
        $this->assertRegexp('/[^a-z0-9]?www\\.example\\.com$/i', $route->assembly());
    }

    public function testAssemblyHost()
    {
        $route = $this->buildHostnameRule();
        $this->assertRegexp('/[^a-z0-9]?foo\\.example\\.com$/i', $route->assembly(array('subdomain' => 'foo')));
    }
    ...
    public function testAssemblyHostWithDefaultParam()
    {
        ...
        $this->assertRegexp('/[^a-z0-9]?bar\\.example\\.com$/i', $route->assembly());
    }

    public function testAssemblyHostWithDefaultParamInSecureScheme()
    {
        $route = $this->buildHostnameRuleWithDefault()->setScheme(RouterHostnameRule::SCHEME_HTTPS);
        $this->assertRegexp('/^https\\:\\/\\/bar\\.example\\.com$/i', $route->assembly());
    }
    ...
    public function testRouteWithHostname()
    {
        ...
        $route                = RouterHostnameRule::create('www.example.com')
                                                  ->setDefaults(array('area' => 'host-foo', 'action' => 'host-bar'));
    ...
    {
        $route = RouterHostnameRule::create('www.example.com')->setDefaults(array('area' => 'ctrl', 'action' => 'act'));
        return $route;
    }
    ...
    {
        $route = RouterHostnameRule::create(':subdomain.example.com')
                                   ->setDefaults(array('area' => 'ctrl', 'action' => 'act'))
                                   ->setRequirements(array('subdomain' => '(foo|bar)'));
        return $route;
    }
    ...
    {
        $route = RouterHostnameRule::create(':subdomain.example.com')
                                   ->setDefaults(array('area' => 'ctrl', 'action' => 'act', 'subdomain' => 'bar'))
                                   ->setRequirements(array('subdomain' => '(foo|bar)'));
        return $route;
    }
