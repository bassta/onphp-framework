...
class RouterTransparentRuleTest extends TestCase
{
    ...
    public function testStaticPathLongerThanParts()
    {
        ...
        $values = $route->match($this->buildRequest('http://localhost/users/a/martel'));
        $this->assertEquals(array(), $values);
    }

    public function testStaticMatchWithDefaults()
    {
        $route  = RouterTransparentRule::create('users/all')->setDefaults(array('area' => 'ctrl'));
        $values = $route->match($this->buildRequest('http://localhost/users/all'));
    ...
    {
        $route  = RouterTransparentRule::create(':area/:action')
                                       ->setDefaults(array('area' => 'index', 'action' => 'index'));
        $values = $route->match($this->buildRequest('http://localhost/archive/action/bogus'));
        $this->assertEquals(array(), $values);
    }
    ...
    public function testNotMatchedWithVariablesAndStatic()
    {
        ...
        $values = $route->match($this->buildRequest('http://localhost/ctrl/act/2000'));
        $this->assertEquals(array(), $values);
    }

    public function testStaticMatchWithWildcard()
    {
        $route  = RouterTransparentRule::create('news/view/*')
                                       ->setDefaults(array('area' => 'news', 'action' => 'view'));
        $values = $route->match($this->buildRequest('http://localhost/news/view/show/all/year/2000/empty'));
    ...
    {
        $route  = RouterTransparentRule::create('news/*')->setDefaults(array('area' => 'news', 'action' => 'view'));
        $values = $route->match($this->buildRequest('http://localhost/news/klucz/'.urlencode('wartość').'/'.urlencode('wskaźnik').'/'.urlencode('wartość')));
        $this->assertEquals('news', $values['area']);
        $this->assertEquals('view', $values['action']);
    ...
    {
        $route  = RouterTransparentRule::create('news/*')->setDefaults(array('area' => 'news', 'action' => 'view'));
        $values = $route->match($this->buildRequest('http://localhost/news/wska%C5%BAnik/warto%C5%9B%C4%87'));
        $this->assertEquals('news', $values['area']);
        $this->assertEquals('view', $values['action']);
        $this->assertEquals('wartość', $values['wskaźnik']);
    }
    ...
    public function testVariableUTFValues()
    {
        ...
        $values = $route->match($this->buildRequest('http://localhost/test/'.urlencode('aä')));
        $this->assertEquals('aä', $values['param']);
    }

    public function testOneVariableValue()
    {
        $route  = RouterTransparentRule::create(':action')->setDefaults(array('area' => 'ctrl', 'action' => 'action'));
        $values = $route->match($this->buildRequest('http://localhost/act'));
        $this->assertEquals('ctrl', $values['area']);
        $this->assertEquals('act', $values['action']);
    }
    ...
    {
        $route  = RouterTransparentRule::create(':area/:action/:year')->setDefaults(array('year' => '2006'));
        $values = $route->match($this->buildRequest('http://localhost/ctrl/act'));
        $this->assertEquals('ctrl', $values['area']);
        $this->assertEquals('act', $values['action']);
    ...
    {
        $route  = RouterTransparentRule::create(':area/:action/:year')->setDefaults(array('year' => null));
        $values = $route->match($this->buildRequest('http://localhost/ctrl/act'));
        $this->assertEquals('ctrl', $values['area']);
        $this->assertEquals('act', $values['action']);
    ...
    {
        $route  = RouterTransparentRule::create(':area/:action/:year')->setRequirements(array('year' => '\\d+'));
        $values = $route->match($this->buildRequest('http://localhost/ctrl/act/2000'));
        $this->assertEquals('ctrl', $values['area']);
        $this->assertEquals('act', $values['action']);
        $this->assertEquals('2000', $values['year']);
    ...
    {
        $route  = RouterTransparentRule::create(':area/:action/:year')->setRequirements(array('year' => '\\d+'));
        $values = $route->match($this->buildRequest('http://localhost/ctrl/act/2000t'));
        $this->assertEquals(array(), $values);
    }
    ...
    public function testVariablesWithWildcardAndNumericKey()
    {
        ...
        $values = $route->match($this->buildRequest('http://localhost/c/a/next/2000/show/all/sort/name'));
    ...
    {
        $route = RouterTransparentRule::create('authors/:name')->setDefaults(array('name' => 'martel'));
        $url   = $route->assembly();
    ...
    {
        $route  = RouterTransparentRule::create('archive/:year/*')
                                       ->setDefaults(array('area' => 'archive', 'action' => 'show'));
        $values = $route->match($this->buildRequest('http://localhost/archive/2006/show/all/sort/name'));
    ...
    public function testAssemblyWithReset2()
    {
        $route  = RouterTransparentRule::create(':area/:action/*')
                                       ->setDefaults(array('area' => 'archive', 'action' => 'show'));
        $values = $route->match($this->buildRequest('http://localhost/users/list'));
    ...
    {
        $route  = RouterTransparentRule::create('archive/:year/*')
                                       ->setDefaults(array('area' => 'archive', 'action' => 'show', 'year' => 2005));
        $values = $route->match($this->buildRequest('http://localhost/archive/2006/show/all/sort/name'));
        $url    = $route->assembly(array(), true);
    ...
    {
        $route  = RouterTransparentRule::create('*')->setDefaults(array('area' => 'index', 'action' => 'index'));
        $values = $route->match($this->buildRequest('http://localhost/key1/value1/key2/value2'));
    ...
    {
        $route = new RouterTransparentRule('authors/:name/*');
        $url   = $route->assembly(array('name' => 'martel', 'var' => 'value'));
    ...
    public function testAssemblyWithUrlVariablesReuse()
    {
        ...
        $values = $route->match($this->buildRequest('http://localhost/archives/2006/07'));
    ...
    {
        $route  = RouterTransparentRule::create('archives/:year/:month/*')->setDefaults(array('area' => 'archive'));
        $values = $route->match($this->buildRequest('http://localhost/archives/2006/07/controller/test/year/10000/sort/author'));
        $this->assertInternalType('array', $values);
    ...
    {
        $route  = RouterTransparentRule::create('users/all')->setDefaults(array('area' => 'ctrl', 'action' => 'act'));
    ...
    {
        $routeConf = array('route' => 'users/all', 'defaults' => array('area' => 'ctrl'));
        $route     = RouterTransparentRule::create($routeConf['route'])->setDefaults($routeConf['defaults']);
    ...
    public function testAssemblyResetDefaults()
    {
        $route  = RouterTransparentRule::create(':area/:action/*')
                                       ->setDefaults(array('area' => 'index', 'action' => 'index'));
        $values = $route->match($this->buildRequest('http://localhost/news/view/id/3'));
    ...
    {
        $route = RouterTransparentRule::create('families/:action/*')
                                      ->setDefaults(array('module' => 'default', 'area' => 'categories', 'action' => 'index'));
        $this->assertEquals('families', $route->assembly());
        $values = $route->match($this->buildRequest('http://localhost/families/edit/id/4'));
        $this->assertInternalType('array', $values);
    ...
    {
        $router = RouterTransparentRule::create('contest/:contest/*')->setDefaults(array('area' => 'contestController'))
                                       ->setRequirements(array('contest' => '\\d+'));
        $this->assertEquals('contest/123/param/435', $router->assembly(array('area' => 'contestController', 'contest' => 123, 'param' => 435), 'contest'));
    }

    public function testAssemblingTransparentDefaultLogic()
    {
        $transparent = RouterTransparentRule::create(':bar/:area/:action')
                                            ->setDefaults(array('bar' => 'barvalue', 'area' => 'controller', 'action' => 'create'));
        $this->assertEquals('barvalue/controller/misc', $transparent->assembly(array('action' => 'misc')));
        $this->assertEquals('barvalue/misc', $transparent->assembly(array('area' => 'misc')));
        $this->assertEquals('misc', $transparent->assembly(array('bar' => 'misc')));
    }
    ...
    public function testEncode()
    {
        $route = RouterTransparentRule::create(':area/:action/*')
                                      ->setDefaults(array('area' => 'index', 'action' => 'index'));
        $url   = $route->assembly(array('area' => 'My Controller'), false, true);
        $this->assertEquals('My+Controller', $url);
        $url = $route->assembly(array('area' => 'My Controller'), false, false);
        $this->assertEquals('My Controller', $url);
        $token = $route->match($this->buildRequest('http://localhost/en/foo/id/'.urlencode('My Value')));
        $url   = $route->assembly(array(), false, true);
        $this->assertEquals('en/foo/id/My+Value', $url);
        $url = $route->assembly(array('id' => 'My Other Value'), false, true);
        $this->assertEquals('en/foo/id/My+Other+Value', $url);
        $route = RouterTransparentRule::create(':area/*')->setDefaults(array('area' => 'My Controller'));
