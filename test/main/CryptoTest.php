...
{
    public function runDiffieHellmanExchange(BigNumberFactory $factory, RandomSource $source)
    {
        $parameters = DiffieHellmanParameters::create($factory->makeNumber(2), $factory->makeNumber('155172898181473697471232257763715539915724801'.'966915404479707795314057629378541917580651227423'.'698188993727816152646631438561595825688188889951'.'272158842675419950341258706556549803580104870537'.'681476726513255747040765857479291291572334510643'.'245094715007229621094194349783925984760375594985'.'848253359305585439638443'));
        $sideA      = DiffieHellmanKeyPair::generate($parameters, $source);
        $sideB      = DiffieHellmanKeyPair::generate($parameters, $source);
        $this->assertEquals($sideA->makeSharedKey($sideB->getPublic())
                                  ->toString(), $sideB->makeSharedKey($sideA->getPublic())->toString());
    }
    ...
    public function runDiffieHellmanGeneration(BigNumberFactory $factory)
    {
        ...
        $this->assertEquals($bigPair->getPrivate()
                                    ->toString(), '');
        $this->assertEquals($bigPair->getPublic()
                                    ->toString(), '');
        $this->assertEquals($bigPair->makeSharedKey($factory->makeFromBinary(base64_decode('ALOlru0GPBCbWulLlZPjRFCVPQDOnmQ+bUaowbHvgA4D1TEDlHA0WgX+HnQuq3KleYWK8jgY0nH/l02gdE93OCMq1Kitat+I8PE1HGVkAQ1J7pfM6f3WISSCa88xm63CLVg4MPTCP+0ONh6A5XUkN+D+LwS4ff9zUoF9GVRRKN6K')))
                                    ->toString(), '');
    }
