...
final class EntityProtoConvertersTest extends PHPUnit_Framework_TestCase
{
    public function testDirectoryBinder()
    {
        ...
        $unconverter         = ObjectToDirectoryBinder::create(Singleton::getInstance('EntityProtoDirectoryItem'))
                                                      ->setDirectory($newDirectoryContext);
        $unconverter->make($result);
        $this->assertEquals(file_get_contents($directoryContext.'/contents'), file_get_contents($newDirectoryContext.'/contents'));
    }
    ...
    public function testSymlinks()
    {
        ...
        $actual   = glob($ringDir.'/*');
        $expected = array($ringDir.'/contents', $ringDir.'/inner', $ringDir.'/items', $ringDir.'/textField');
        $this->assertEquals($expected, $actual);
        $actual   = glob($ringDir.'/items/*');
        $expected = array($ringDir.'/items/421', $ringDir.'/items/422', $ringDir.'/items/423', $ringDir.'/items/424');
        $this->assertEquals($expected, $actual);
        $actual   = glob($ringDir.'/items/424/*');
        $expected = array($ringDir.'/items/424/contents', $ringDir.'/items/424/inner', $ringDir.'/items/424/textField');
        $this->assertEquals($expected, $actual);
    ...
    {
        $ringDir        = ONPHP_TEMP_PATH.'tests/main/data/ring';
        $converter      = ObjectToDirectoryBinder::create(Singleton::getInstance('EntityProtoDirectoryItem'))
        ...
        $ringListHead->setInner($items[2] = DirectoryItem::create()->setId('422')
                                                         ->setInner($items[1] = DirectoryItem::create()->setId('423')
                                                                                             ->setInner($items[0] = DirectoryItem::create()
                                                                                                                                 ->setId('424')
...
final class EntityProtoDirectoryItem extends EntityProto
{
    ...
    {
        return array('items' => Primitive::formsList('items')->of('DirectoryItem')
                                         ->required(), 'textField' => Primitive::string('textField')->setMax(256)
                                                                               ->optional(), 'fileName' => Primitive::file('contents')
                                                                                                                    ->required(), 'inner' => Primitive::form('inner')
                                                                                                                                                      ->of('DirectoryItem')
                                                                                                                                                      ->optional());
    }
}