...
final class CurlHttpClientTest extends TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->assertEquals($this->generateString(array(), array(), array(), ''), self::$emptyMsg, 'wrong server empty response');
    }

    public function testGetWithAdditionalGet()
    {
        $get      = array('a' => array('b@&=' => array('c' => '@d@[]')), 'e' => array('f' => array('&1&', '3', '5')));
        $request  = $this->spawnRequest(HttpMethod::get(), 'urlGet=really')->setGet($get)
    ...
    public function testPostAndFilesWithMultiCurl()
    {
        ...
        $this->assertEquals($this->generateString(array('urlGet' => 'super') + $get, $post1, array(), UrlParamsUtils::toString($post1)), $client->getResponse($request1)
                                                                                                                                                ->getBody());
        ...
        $this->assertEquals($this->generateString(array(), $post2, $filesExpectation, ''), $client->getResponse($request2)
                                                                                                  ->getBody());
        //check response 3rd request
        $this->assertEquals($this->generateString(array(), array(), array(), $body), $client->getResponse($request3)
                                                                                            ->getBody());
    }

    public function testSecurityExceptionWithSendingFileAndAtInPost()
    {
        $post    = array('a' => array(array('b' => '@foobar')));
        $files   = array('file' => $this->getFile1Path());
        $request = $this->spawnRequest(HttpMethod::post())->setPost($post)->setFiles($files);
        try {
