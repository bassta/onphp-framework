...
final class AutoloaderClassPathCacheTest extends TestCase
{
    ...
    public function testRecacheOnChangedPath()
    {
        ...
        $service->setNamespaceResolver($this->spawnResolver(array('getPaths' => array('' => array('path1')))));
        $service->autoload('Form');
    }
    ...
    private function spawnService(array $options = array())
    {
        $service = $this->getMockBuilder('AutoloaderClassPathCache')
                        ->setMethods(array('includeFile', 'register', 'unregister'))->getMock();
        /* @var $service AutoloaderClassPathCache */
        $service->setNamespaceResolver($this->spawnResolver($options))->setClassCachePath($this->spawnCacheDir());
        return $service;
    }
    ...
    {
        $options += array('getPaths' => array('' => array('path1', 'path2')), 'getPathsCount' => 1, 'getClassExtension' => EXT_CLASS, 'getClassPathList' => array(0 => 'path1/', '\\Form' => 0, 1 => 'path1/path2/', '\\Sub\\Form' => 1), 'getClassPathListCount' => 1);
        $mock = $this->getMock('NamespaceResolver');
        $mock->expects($this->any())->method('getClassExtension')
             ->will($this->returnValue($options['getClassExtension']));
        $mock->expects($this->exactly($options['getPathsCount']))->method('getPaths')
             ->will($this->returnValue($options['getPaths']));
        $mock->expects($this->exactly($options['getClassPathListCount']))->method('getClassPathList')
