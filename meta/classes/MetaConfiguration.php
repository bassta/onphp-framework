...
final class MetaConfiguration extends Singleton implements Instantiatable
{
    ...
    public function load($metafile, $generate = true)
    {
        ...
        foreach ($this->classes as $name => $class) {
            ...
            if (isset($sourceLink)) {
                Assert::isTrue(isset($this->sources[$sourceLink]), "unknown source '{$sourceLink}' specified "."for class '{$name}'");
        ...
        foreach ($this->liaisons as $class => $parent) {
            if (isset($this->classes[$parent])) {
                Assert::isFalse($this->classes[$parent]->getTypeId() == MetaClassType::CLASS_FINAL, "'{$parent}' is final, thus can not have childs");
                if ($this->classes[$class]->getPattern() instanceof DictionaryClassPattern) {
                    throw new UnsupportedMethodException('DictionaryClass pattern does '.'not support inheritance');
                }
                $this->classes[$class]->setParent($this->classes[$parent]);
            } else {
                throw new MissingElementException("unknown parent class '{$parent}'");
            }
        }
        ...
        foreach ($this->references as $className => $list) {
            ...
            foreach ($list as $refer) {
                ...
                if ($remote->getPattern() instanceof ValueObjectPattern && isset($this->references[$refer])) {
        ...
        foreach ($this->classes as $class) {
            foreach ($class->getProperties() as $property) {
                if ($property->getRelation() && $property->getRelationId() != MetaRelation::ONE_TO_ONE) {
                    ...
                    $oldStlye = ONPHP_META_DAO_DIR.$class->getName().'To'.$property->getType()
                                                                                   ->getClassName().'DAO'.EXT_CLASS;
    ...
    public function checkIntegrity()
    {
        ...
        foreach ($this->classes as $name => $class) {
            if (!($class->getPattern() instanceof SpookedClassPattern || $class->getPattern() instanceof SpookedEnumerationPattern || $class->getPattern() instanceof SpookedEnumPattern || $class->getPattern() instanceof InternalClassPattern) && class_exists($class->getName(), true)) {
                ...
                $dao = $object->dao();
                Assert::isEqual($dao->getIdName(), $class->getIdentifier()
                                                         ->getColumnName(), 'identifier name mismatch in '.$class->getName().' class');
                try {
                    DBPool::getByDao($dao);
                } catch (MissingElementException $e) {
                    // skipping
                ...
                $query = Criteria::create($dao)->setLimit(1)->add(Expression::notNull($class->getIdentifier()
                                                                                            ->getName()))
                                 ->addOrder($class->getIdentifier()->getName())->toSelectQuery();
                ...
                $out->warning('/');
                if (Criteria::create($dao)->setFetchStrategy(FetchStrategy::cascade())
                            ->toSelectQuery() == $dao->makeSelectHead()
                ) {
                    $out->info('H', true);
                } else {
                    $out->error('H', true);
    ...
    public function checkForStaleFiles($drop = false)
    {
        ...
        return $this->checkDirectory(ONPHP_META_AUTO_BUSINESS_DIR, 'Auto', null, $drop)
    ...
    public function getClassByName($name)
    {
        ...
        throw new MissingElementException("knows nothing about '{$name}' class");
    }
    ...
    {
        $out = $this->getOutput();
        foreach (glob($directory.'*.class.php', GLOB_NOSORT) as $filename) {
    ...
    private function addSource(SimpleXMLElement $source)
    {
        ...
        $default = isset($source['default']) && (string)$source['default'] == 'true' ? true : false;
        Assert::isFalse(isset($this->sources[$name]), "duplicate source - '{$name}'");
        Assert::isFalse($default && $this->defaultSource !== null, 'too many default sources');
    ...
    {
        if ((!$class->getParent() || $class->getFinalParent()
                                           ->getPattern() instanceof InternalClassPattern) && !$class->getPattern() instanceof ValueObjectPattern && !$class->getPattern() instanceof InternalClassPattern
        ) {
        ...
        foreach ($class->getProperties() as $property) {
            if (!$property->getType()->isGeneric() && $property->getType() instanceof ObjectType && $property->getType()
                                                                                                             ->getClass()
    ...
    private function processIncludes(SimpleXMLElement $xml, $metafile)
    {
        foreach ($xml->include as $include) {
            ...
            Assert::isTrue(is_readable($path), 'can not include '.$file);
            $this->getOutput()->infoLine('Including "'.$path.'".')->newLine();
    ...
    private function processClasses(SimpleXMLElement $xml, $metafile, $generate)
    {
        foreach ($xml->classes[0] as $xmlClass) {
            ...
            if (isset($xmlClass['type'])) {
                ...
                if ($type == 'spooked') {
                    $this->getOutput()->warning($class->getName(), true)
                         ->warningLine(': uses obsoleted "spooked" type.')->newLine();
                }
            ...
            if ($class->getPattern() instanceof InternalClassPattern) {
                Assert::isTrue($metafile === ONPHP_META_PATH.'internal.xml', 'internal classes can be defined only in onPHP, sorry');
            ...
            foreach ($xmlClass->properties[0] as $xmlProperty) {
                ...
                if (isset($xmlProperty['column'])) {
                    ...
                    if (isset($this->classes[$property->getType()->getClassName()]) && $property->getType()->getClass()
                ...
                if (!$property->getType()->isGeneric()) {
                    if (!isset($xmlProperty['relation'])) {
                        ...
                        if ($property->getRelationId() == MetaRelation::ONE_TO_ONE && $property->getFetchStrategyId() != FetchStrategy::LAZY && $property->getType()
        ...
        foreach ($rows as $row) {
            if (!isset($ids[$row['id']])) {
                ...
                if ($ids[$row['id']] != $row['name']) {
                    ...
                    $updateQueries .= OSQL::update($tableName)->set('name', $ids[$row['id']])
                                          ->where(Expression::eq('id', $row['id']))->toDialectString($db->getDialect()).';
';
                }
                unset($ids[$row['id']]);
            }
