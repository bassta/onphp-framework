...
class MetaClassProperty
{
    ...
    public function getFetchStrategyId()
    {
        if ($this->strategy) {
            return $this->strategy->getId();
                       ...
                       ->getFetchStrategyId() && $this->getRelationId() == MetaRelation::ONE_TO_ONE && $this->getType() instanceof ObjectType && !$this->getType()
                                                                                                                                                       ->isGeneric()
        ) {
    ...
    {
        if ($this->getType() instanceof ObjectType && ($this->getType() instanceof InternalType || !$this->getType()
                                                                                                         ->isGeneric() && $this->getType()
                                                                                                                               ->getClass()
    ...
    public function toLightProperty(MetaClass $holder)
    {
        ...
        if ($this->getRelationId() == MetaRelation::ONE_TO_MANY || $this->getRelationId() == MetaRelation::MANY_TO_MANY) {
            ...
            if ($pattern instanceof EnumerationClassPattern) {
                ...
                $primitiveName = 'enum';
            } elseif ($pattern instanceof DictionaryClassPattern && ($identifier = $this->getType()->getClass()
                                                                                        ->getIdentifier())
            ) {
        ...
        return call_user_func_array(array($propertyClassName, 'fill'), array(new $propertyClassName(), $this->getName(), $this->getName() != $this->getRelationColumnName() ? $this->getRelationColumnName() : null, $primitiveName, $className, $size, $this->isRequired(), $this->getType()
                                                                                                                                                                                                                                                                                  ->isGeneric(), $inner, $this->getRelationId(), $this->getFetchStrategyId()));
    }
