...
class PrimitiveDate extends ComplexPrimitive
{
    ...
    {
        if ($this->getState()->isFalse() || $this->getState()->isNull()) {
    ...
    public function importValue($value)
    {
        ...
        $marriedRaw  = array(self::DAY => $value->getDay(), self::MONTH => $value->getMonth(), self::YEAR => $value->getYear());
    ...
    public function exportValue()
    {
        if ($this->value === null) {
            if ($this->getState()->isTrue()) {
                ...
                return array(self::DAY => null, self::MONTH => null, self::YEAR => null);
            }
        }
        if ($this->getState()->isTrue()) {
            return $this->value->toString();
        } else {
            return array(self::DAY => $this->value->getDay(), self::MONTH => $this->value->getMonth(), self::YEAR => $this->value->getYear());
        }
    }
