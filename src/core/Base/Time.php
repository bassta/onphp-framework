...
final class Time implements Stringable
{
    ...
    public function __construct($input)
    {
        ...
        if (count($time) === 2) {
            ...
            switch ($lenght) {
                ...
                case 5:
                case 6:
                    $this->setHour(substr($input, 0, 2))->setMinute(substr($input, 2, 2))
    ...
    {
        $hour = (int)$hour;
        Assert::isTrue($hour >= 0 && $hour <= 24, 'wrong hour specified');
        $this->hour   = $hour;
        $this->string = null;
        return $this;
    }
    ...
    {
        $minute = (int)$minute;
        Assert::isTrue($minute >= 0 && $minute <= 60, 'wrong minute specified');
        $this->minute = $minute;
        $this->string = null;
        return $this;
    }
    ...
    {
        $second = (int)$second;
        Assert::isTrue($second >= 0 && $second <= 60, 'wrong second specified');
        $this->second = $second;
        $this->string = null;
        return $this;
    }
    ...
    {
        return $this->toString($delimiter).$delimiter.($this->second ? $this->doublize($this->second) : '00');
    }
    ...
    {
        if ($this->string === null) {
            $this->string = $this->doublize($this->hour).$delimiter.$this->doublize($this->minute);
        }
        return $this->string;
    }
    ...
    {
        return $this->hour * 60 + $this->minute + round($this->second / 100, 0);
    }
