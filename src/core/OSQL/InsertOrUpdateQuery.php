...
abstract class InsertOrUpdateQuery extends QuerySkeleton implements SQLTableName
{
     ...
     * @return InsertOrUpdateQuery
     **/
    public function lazySet($field, $object = null)
    {
        if ($object === null) {
            $this->set($field, null);
            ...
            $this->set($field.'_min', $object->getMin())->set($field.'_max', $object->getMax());
        } elseif ($object instanceof DateRange) {
            $this->set($field.'_start', $object->getStart())->set($field.'_end', $object->getEnd());
