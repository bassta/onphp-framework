...
final class SelectQuery extends QuerySkeleton implements Named, JoinCapableQuery, Aliased
{
    ...
    {
        $this->joiner->fullOuterJoin(new SQLFullOuterJoin($table, $logic, $alias));
        $this->aliases[$alias] = true;
        return $this;
    }
    ...
    public function groupBy($field, $table = null)
    {
        if ($field instanceof DialectString) {
            ...
            $this->group[] = new DBField($field, $this->getLastTable($table));
        }
        return $this;
    }
    ...
    {
        $this->fields[] = $this->resolveSelectField($field, $alias, $this->getLastTable());
                ...
                if ($array[$i] instanceof DialectString) {
                    if ($array[$i] instanceof DBField) {
                        ...
                        if ($array[$i] instanceof SQLFunction) {
                            $alias = $array[$i]->setAlias($prefix.$array[$i]->getName())->getAlias();
                        } else {
                            $alias = $array[$i];
                        }
                    }
    ...
    {
        if ($field instanceof OrderBy || $field instanceof DialectString) {
            return $field;
        } else {
            return new OrderBy(new DBField($field, $this->getLastTable($table)));
        }
    }
}