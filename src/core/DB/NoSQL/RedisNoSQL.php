...
final class RedisNoSQL extends CachePeer implements ListGenerator
{
     ...
     * @return RedisNoSQL
     */
    public static function create($host = self::DEFAULT_HOST, $port = self::DEFAULT_PORT, $timeout = self::DEFAULT_TIMEOUT)
    {
        return new self($host, $port, $timeout);
    }

    public function __construct($host = self::DEFAULT_HOST, $port = self::DEFAULT_PORT, $timeout = self::DEFAULT_TIMEOUT)
    {
        $this->host    = $host;
        $this->port    = $port;
