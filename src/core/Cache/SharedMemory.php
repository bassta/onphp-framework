...
final class SharedMemory extends SelectivePeer
{
     ...
     * @return SharedMemory
     **/
    public static function create($defaultSize = self::DEFAULT_SEGMENT_SIZE, $customSized = array())
    {
     ...
     * @return SharedMemory
     **/
    public function __construct($defaultSize = self::DEFAULT_SEGMENT_SIZE, $customSized = array())
    {
    ...
    public function __destruct()
    {
        ...
        // sync classes
        $segment = shm_attach(self::INDEX_SEGMENT, self::DEFAULT_SEGMENT_SIZE, ONPHP_IPC_PERMS);
        try {
    ...
    protected function store($action, $key, $value, $expires = 0)
    {
        ...
        try {
            shm_put_var($segment, $this->stringToInt($key), array('value' => $this->prepareData($value), 'expires' => $expires));
            return true;
        } catch (BaseException $e) {
            // not enough memory
            return false;
    ...
    private function getSegment()
    {
        ...
        if (!isset(self::$attached[$class])) {
            self::$attached[$class] = shm_attach($this->stringToInt($class), isset($this->customSized[$class]) ? $this->customSized[$class] : $this->defaultSize, ONPHP_IPC_PERMS);
        }
