...
final class DebugCachePeer extends SelectivePeer
{
     ...
     **/
    public static function create(CachePeer $peer, $logfile, $isWeb = true, $appendFile = true)
    {
    ...
    private function log($action, $totalTime, $key = null)
    {
        if ($this->actionFilter) {
            if (!empty($this->blackListActions) && in_array($action, $this->blackListActions)) {
                return $this;
            }
            if (!empty($this->whiteListActions) && !in_array($action, $this->whiteListActions)) {
                return $this;
            }
        }
        $record = null;
        if ($this->isWeb) {
            $record .= (isset($_SERVER['SSI_REQUEST_URI']) ? $_SERVER['SSI_REQUEST_URI'] : $_SERVER['REQUEST_URI']).'	';
        }
