...
final class RubberFileSystem extends CachePeer
{
    ...
    public function isAlive()
    {
        if (!is_writable($this->directory)) {
            return mkdir($this->directory, 448, true);
        } else {
            return true;
        }
    }
    ...
    protected function store($action, $key, $value, $expires = 0)
    {
        ...
        if ($action == 'add' && is_readable($path) && filemtime($path) > $time) {
            return true;
        }
    ...
    {
        return $this->directory.$key[0].$key[1].DIRECTORY_SEPARATOR.substr($key, 2);
    }
}