...
final class OqlSelectParser extends OqlParser
{
    ...
    private function limitState()
    {
        if ($this->checkKeyword($this->tokenizer->peek(), 'limit')) {
            ...
            if ($this->checkToken($token, OqlToken::NUMBER) || $this->checkToken($token, OqlToken::SUBSTITUTION)) {
                $this->oqlObject->setLimit($this->makeQueryParameter($token));
            } else {
    ...
    private function offsetState()
    {
        if ($this->checkKeyword($this->tokenizer->peek(), 'offset')) {
            ...
            $token = $this->tokenizer->next();
            if ($this->checkToken($token, OqlToken::NUMBER) || $this->checkToken($token, OqlToken::SUBSTITUTION)) {
                $this->oqlObject->setOffset($this->makeQueryParameter($token));
            } else {
