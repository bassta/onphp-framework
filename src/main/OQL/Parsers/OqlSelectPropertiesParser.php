...
final class OqlSelectPropertiesParser extends OqlParser
{
    ...
    protected function handleState()
    {
        if ($this->state == self::INITIAL_STATE) {
            $list = $this->getCommaSeparatedList(array($this, 'getArgumentExpression'), 'expecting expression or aggregate function call');
    ...
    protected function getArgumentExpression()
    {
        ...
        if ($this->checkToken($token, OqlToken::AGGREGATE_FUNCTION)) {
            ...
            if ($this->openParentheses(false)) {
                ...
                return $this->makeQueryExpression(self::$classMap[$functionName], $expression, $this->getAlias());
            } else {
                $this->tokenizer->back();
            }
        }
        ...
        return $this->makeQueryExpression(self::$classMap[self::PROPERTY_PROJECTION], $this->getLogicExpression(), $this->getAlias());
    }
    ...
    private function getAlias()
    {
        if ($this->checkKeyword($this->tokenizer->peek(), 'as')) {
            $this->tokenizer->next();
            if (!($alias = $this->tokenizer->next()) || !$this->checkIdentifier($alias)) {
                $this->error('expecting alias name:', $this->getTokenValue($alias, true));
            }
            return $alias;
        }
        return null;
