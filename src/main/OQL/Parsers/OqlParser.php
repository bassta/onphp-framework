...
abstract class OqlParser
{
    ...
    private static $logicPriorityMap = array(self::LOGIC_PRIORITY_OR => 'or', self::LOGIC_PRIORITY_AND => 'and', self::LOGIC_PRIORITY_LT_GT => array('>', '<', '>=', '<='), self::LOGIC_PRIORITY_EQ => array('=', '!='), self::LOGIC_PRIORITY_TERMINAL => null);
    ...
    private static $arithmeticPriorityMap = array(self::ARITHMETIC_PRIORITY_ADD => array('+', '-'), self::ARITHMETIC_PRIORITY_MUL => array('*', '/'), self::ARITHMETIC_PRIORITY_TERMINAL => null);
     ...
     **/
    protected abstract function makeOqlObject();

    protected abstract function handleState();
    ...
    protected function getTokenValue($token, $raw = false)
    {
        if ($token instanceof OqlToken) {
            return $raw ? $token->getRawValue() : $token->getValue();
        }
        return null;
    }
    ...
    {
        if ($token instanceof OqlToken && $token->getType() == $type) {
            if ($value === null) {
                return true;
    ...
    protected function checkIdentifier($token)
    {
        if ($token instanceof OqlToken) {
            if ($token->getType() == OqlToken::IDENTIFIER) {
                return true;
            } elseif ($token->getType() == OqlToken::KEYWORD || $token->getType() == OqlToken::AGGREGATE_FUNCTION) {
    ...
    {
        return $token instanceof OqlToken && ($token->getType() == OqlToken::STRING || $token->getType() == OqlToken::NUMBER || $token->getType() == OqlToken::BOOLEAN || $token->getType() == OqlToken::NULL || $token->getType() == OqlToken::SUBSTITUTION);
    }
    ...
    {
        if ($this->checkToken($this->tokenizer->peek(), OqlToken::PARENTHESES, '(')) {
            $this->tokenizer->next();
            $this->parentheses++;
            return true;
        } elseif ($required) {
    ...
    protected function getConstantExpression()
    {
        ...
        $token = $this->tokenizer->peek();
        if ($token instanceof OqlToken && (!$isUnaryMinus && ($token->getType() == OqlToken::STRING || $token->getType() == OqlToken::BOOLEAN || $token->getType() == OqlToken::NULL) || ($token->getType() == OqlToken::NUMBER || $token->getType() == OqlToken::SUBSTITUTION))) {
            $this->tokenizer->next();
            return $this->makeQuerySignedExpression($token, $isUnaryMinus);
        }
        return null;
     ...
     **/
    protected function getLogicExpression($priority = self::LOGIC_PRIORITY_LOWEST)
    {
        $expression = null;
        ...
        if ($priority == self::LOGIC_PRIORITY_TERMINAL) {
            ...
            if ($this->checkKeyword($token, 'not')) {
                $this->tokenizer->next();
                if ($argument = $this->getLogicExpression(self::LOGIC_PRIORITY_UNARY_NOT)) {
                    return $this->makeQueryExpression(self::$classMap[self::PREFIX_UNARY_EXPRESSION], PrefixUnaryExpression::NOT, $argument);
                } else {
                    $this->error('expecting argument in expression: not');
                }
            }
            // first argument
            if (!($expression = $this->getIdentifierExpression()) && !($expression = $this->getConstantExpression())) {
                $this->error('expecting first argument in expression:', $this->getTokenValue($this->tokenizer->peek(), true));
            }
            ...
            if (!$isNot && $this->checkKeyword($operator, 'is')) {
                $this->tokenizer->next();
                $logic = null;
                ...
                if ($this->checkToken($this->tokenizer->peek(), OqlToken::NULL)) {
                    $this->tokenizer->next();
                    $logic = $isNot ? PostfixUnaryExpression::IS_NOT_NULL : PostfixUnaryExpression::IS_NULL;
                } elseif (!$isNot && $this->checkToken($this->tokenizer->peek(), OqlToken::BOOLEAN)) {
                    $logic = $this->tokenizer->next()
                                             ->getValue() === true ? PostfixUnaryExpression::IS_TRUE : PostfixUnaryExpression::IS_FALSE;
                }
                if ($logic) {
                    return $this->makeQueryExpression(self::$classMap[self::POSTFIX_UNARY_EXPRESSION], $expression, $logic);
                } else {
                ...
                $list = $this->getCommaSeparatedList(array($this, 'getConstantExpression'), 'expecting constant or substitution in expression: '.$isNotString.'in');
                if (is_array($list) && count($list) == 1) {
                    $list = reset($list);
                }
                $this->closeParentheses(true, 'in expression: '.$isNotString.'in');
                return new OqlInExpression($expression, $this->makeQueryParameter($list), $isNot ? InExpression::NOT_IN : InExpression::IN);
            } elseif ($this->checkKeyword($operator, array('like', 'ilike', 'similar to'))) {
                $this->tokenizer->next();
                $isNotString = $isNot ? 'not ' : '';
                $argument    = $this->tokenizer->next();
                if ($this->checkToken($argument, OqlToken::STRING) || $this->checkToken($argument, OqlToken::SUBSTITUTION)) {
                    return $this->makeQueryExpression(self::$classMap[self::BINARY_EXPRESSION], $expression, $argument, self::$binaryOperatorMap[$isNotString.$this->getTokenValue($operator)]);
                } else {
                    $this->error('expecting string constant or substitution:', $isNotString.$this->getTokenValue($operator, true));
                }
            } elseif (!$isNot && $this->checkKeyword($operator, 'between')) {
                $this->tokenizer->next();
                if (($argument1 = $this->getIdentifierExpression()) || ($argument1 = $this->getConstantExpression())) {
                    if ($this->checkKeyword($this->tokenizer->next(), 'and')) {
                        if (($argument2 = $this->getIdentifierExpression()) || ($argument2 = $this->getConstantExpression())) {
                            return $this->makeQueryExpression(self::$classMap[self::BETWEEN_EXPRESSION], $expression, $argument1, $argument2);
                        } else {
                            $this->error('expecting second argument in expression: between');
                        }
                    } else {
                        $this->error('expecting \'and\' in expression: between');
                    }
                } else {
                    $this->error('expecting first argument in expression: between');
                }
            }
            if ($isNot) {
            ...
            if (!($expression = $this->getLogicExpression($higherPriority))) {
                $this->error('expecting first argument in expression:', is_array($operatorList) ? implode('|', $operatorList) : $operatorList);
            }
            $tokenType = $priority == self::LOGIC_PRIORITY_OR || $priority == self::LOGIC_PRIORITY_AND ? OqlToken::KEYWORD : OqlToken::COMPARISON_OPERATOR;
            while ($this->checkToken($this->tokenizer->peek(), $tokenType, $operatorList)) {
                $operator = $this->tokenizer->next();
                if ($expression2 = $this->getLogicExpression($higherPriority)) {
                    $expression = $this->makeQueryExpression(self::$classMap[self::BINARY_EXPRESSION], $expression, $expression2, self::$binaryOperatorMap[$operator->getValue()]);
                } else {
                    $this->error('expecting second argument in expression:', $this->getTokenValue($operator, true));
                }
            }
        }
        return $expression;
     ...
     * @return OqlQueryExpression
     **/
    protected function getArithmeticExpression($priority = self::ARITHMETIC_PRIORITY_LOWEST)
    {
        ...
        if ($priority == self::ARITHMETIC_PRIORITY_TERMINAL) {
            ...
            if ($this->openParentheses(false)) {
                ...
                $this->error('expecting argument in expression:', $this->getTokenValue($this->tokenizer->peek(), true));
            }
            ...
            if (!($expression = $this->getArithmeticExpression($higherPriority))) {
                $this->error('expecting first argument in expression:', implode('|', $operatorList));
            }
            while ($this->checkToken($this->tokenizer->peek(), OqlToken::ARITHMETIC_OPERATOR, $operatorList)) {
                $operator = $this->tokenizer->next();
    ...
    protected function getCommaSeparatedList($callback, $message)
    {
        ...
        } while ($isComma = $this->checkToken($this->tokenizer->peek(), OqlToken::PUNCTUATION, ','));
        return $list;
    }
    ...
    protected function makeQueryParameter($argument)
    {
        if ($argument instanceof OqlQueryParameter) {
            ...
            return OqlQueryParameter::create()->setValue($argument->getValue())
    ...
    protected function error($message, $extraMessage = null)
    {
        ...
        throw new SyntaxErrorException($message, $this->tokenizer->getLine(), $this->tokenizer->getPosition());
    }
    ...
    private function isArithmeticExpression()
    {
        ...
        while ($this->checkToken($this->tokenizer->peek(), OqlToken::PARENTHESES, '(')) {
            $this->tokenizer->next();
        }
        // skip unary minus
        ...
        $result = $this->getArithmeticArgumentExpression() && $this->checkToken($this->tokenizer->peek(), OqlToken::ARITHMETIC_OPERATOR);
    ...
    {
        $token = $this->tokenizer->peek();
        if ($this->checkIdentifier($token) || $this->checkToken($token, OqlToken::NUMBER) || $this->checkToken($token, OqlToken::SUBSTITUTION)) {
            $this->tokenizer->next();
