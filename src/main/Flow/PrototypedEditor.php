...
abstract class PrototypedEditor extends MethodMappedController
{
    ...
    public function __construct(Prototyped $subject)
    {
        ...
        $this->map     = MappedForm::create($this->subject->proto()->makeForm())->addSource('id', RequestType::get())
                                   ->setDefaultType(RequestType::post());
        $this->setMethodMapping('drop', 'doDrop')->setMethodMapping('take', 'doTake')
             ->setMethodMapping('save', 'doSave')->setMethodMapping('edit', 'doEdit')->setMethodMapping('add', 'doAdd');
    ...
    public function doDrop(HttpRequest $request)
    {
        ...
        if ($object = $form->getValue('id')) {
            if ($object instanceof Identifiable) {
                ...
                // already deleted
                $form->markMissing('id');
                return ModelAndView::create()->setModel(Model::create()->set('editorResult', self::COMMAND_FAILED)
                                                             ->set('form', $form));
            }
        } else {
            return ModelAndView::create()->setModel(Model::create()->set('editorResult', self::COMMAND_FAILED)
    ...
    public function doTake(HttpRequest $request)
    {
        ...
        if (!$form->getErrors()) {
            ...
            return ModelAndView::create()->setModel(Model::create()->set('id', $object->getId())
                                                         ->set('subject', $object)->set('form', $form)
    ...
    public function doAdd(HttpRequest $request)
    {
        ...
        if (!$form->getErrors()) {
            ...
            return ModelAndView::create()->setModel(Model::create()->set('form', $form)->set('subject', $object)
