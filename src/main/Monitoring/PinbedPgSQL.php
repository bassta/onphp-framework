...
final class PinbedPgSQL extends PgSQL
{
    public function connect()
    {
        if (PinbaClient::isEnabled()) {
            PinbaClient::me()
                       ->timerStart('pg_sql_connect_'.$this->basename, array('group' => 'sql', 'pg_sql_connect' => $this->basename));
        }
    ...
    public function queryRaw($queryString)
    {
        if (PinbaClient::isEnabled()) {
            ...
            PinbaClient::me()
                       ->timerStart('pg_sql_query_'.$this->basename, array('group' => 'sql', 'pg_sql_query' => $queryLabel, 'pg_sql_server' => $this->hostname, 'pg_sql_base' => $this->basename));
        }
        try {
