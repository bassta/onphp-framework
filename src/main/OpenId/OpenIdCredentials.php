...
final class OpenIdCredentials
{
    ...
    public function __construct(HttpUrl $claimedId, HttpClient $httpClient)
    {
        ...
        $response         = $httpClient->send(HttpRequest::create()->setHeaderVar('Accept', self::HEADER_ACCEPT)
                                                         ->setMethod(HttpMethod::get())->setUrl($claimedId));
     ...
     **/
    public static function create(HttpUrl $claimedId, HttpClient $httpClient)
    {
    ...
    {
        $tokenizer  = HtmlTokenizer::create(StringInputStream::create($content))->lowercaseTags(true)
    ...
    {
        $response = $this->httpClient->send(HttpRequest::create()->setHeaderVar('Accept', self::HEADER_ACCEPT)
                                                       ->setMethod(HttpMethod::get())->setUrl(HttpUrl::create()
                                                                                                     ->parse($url)));
        if ($response->getStatus()->getId() != 200) {
            throw new OpenIdException('can\'t fetch document');
        }
