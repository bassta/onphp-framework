<?php

namespace onphp\main\Utils\Logging;

    /***************************************************************************
     *   Copyright (C) 2007 by Ivan Y. Khvostishkov                            *
     *                                                                         *
     *   This program is free software; you can redistribute it and/or modify  *
     *   it under the terms of the GNU Lesser General Public License as        *
     *   published by the Free Software Foundation; either version 3 of the    *
     *   License, or (at your option) any later version.                       *
     *                                                                         *
     ***************************************************************************/
/**
 * @ingroup Utils
 **/
abstract class BaseLogger
{
    private $level = null;

    /**
     * @return Logger
     **/
    public final function severe($message)
    {
        $this->log(LogLevel::severe(), $message);
        return $this;
    }

    /**
     * @return Logger
     **/
    public final function log(LogLevel $level, $message)
    {
        $this->logRecord(LogRecord::create()->setLevel($level)->setMessage($message));
        return $this;
    }

    /**
     * @return Logger
     **/
    public final function logRecord(LogRecord $record)
    {
        $levelMatches = $this->level === null || $record->getLevel()->getId() <= $this->level->getId();
        if ($levelMatches && $this->isLoggable($record)) {
            $this->publish($record);
        }
        return $this;
    }

    /**
     * you may override me
     **/
    protected function isLoggable(LogRecord $record)
    {
        return true;
    }

    /**
     * @return Logger
     **/
    protected abstract function publish(LogRecord $record);

    /**
     * @return Logger
     **/
    public final function warning($message)
    {
        $this->log(LogLevel::warning(), $message);
        return $this;
    }

    /**
     * @return Logger
     **/
    public final function info($message)
    {
        $this->log(LogLevel::info(), $message);
        return $this;
    }

    /**
     * @return Logger
     **/
    public final function config($message)
    {
        $this->log(LogLevel::config(), $message);
        return $this;
    }

    /**
     * @return Logger
     **/
    public final function fine($message)
    {
        $this->log(LogLevel::fine(), $message);
        return $this;
    }

    /**
     * @return Logger
     **/
    public final function finer($message)
    {
        $this->log(LogLevel::finer(), $message);
        return $this;
    }

    /**
     * @return Logger
     **/
    public final function finest($message)
    {
        $this->log(LogLevel::finest(), $message);
        return $this;
    }

    /**
     * @return LogLevel
     **/
    public function getLevel()
    {
        return $this->level;
    }

    /**
     * @return Logger
     **/
    public function setLevel(LogLevel $level)
    {
        $this->level = $level;
        return $this;
    }
}