...
final class AMQPSelective implements AMQPInterface
{
    ...
    {
        if (!isset($this->pool[$name])) {
            throw new MissingElementException("amqp link with name '{$name}' not found");
        }
    ...
    public function getChannel($id)
    {
        ...
        throw new MissingElementException("Can't find AMQP channel with id '{$id}'");
    }
    ...
    {
        if (!isset($this->channels[$id])) {
            throw new MissingElementException("AMQP channel with id '{$id}' not found");
        }
    ...
    protected function processMethod($method)
    {
        ...
        for ($i = 0; $i < count($this->pool); $i++) {
            try {
                ...
                return call_user_func_array(array($this->getCurrentItem(), $method), $args);
