...
abstract class AMQPBaseMessage
{
    ...
    public function setProperties(array $assoc)
    {
        ...
        if (isset($this->properties[self::TIMESTAMP])) {
            $this->timestamp = new Timestamp($this->properties[self::TIMESTAMP]);
        }
        return $this;
    }
    ...
    public function setDeliveryMode($int)
    {
        ...
        Assert::isTrue(in_array($int, array(self::DELIVERY_MODE_NONPERISISTENT, self::DELIVERY_MODE_PERISISTENT)), __METHOD__.": unknown mode {$int}");
