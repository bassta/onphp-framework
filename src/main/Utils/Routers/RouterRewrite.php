...
final class RouterRewrite extends Singleton implements Router, Instantiatable
{
    ...
    public function addRoute($name, RouterRule $route)
    {
        if ($this->hasRoute($name)) {
            throw new RouterException("Route with name '{$name}' is already defined");
        }
     ...
     * @return string
     **/
    public function assembly(array $userParams = array(), $name = null, $reset = false, $encode = true)
    {
        if ($name === null) {
    ...
    {
        return $this->resetBaseUrl()->resetRequest()->resetRoutes();
        return $this;
    }
