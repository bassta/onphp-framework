...
final class RouterRegexpRule extends RouterBaseRule
{
    ...
    public function assembly(array $data = array(), $reset = false, $encode = false)
    {
        if ($this->reverse === null) {
            throw new RouterException('Can not assembly. Reversed route is not specified.');
        }
        ...
        try {
            ...
            throw new RouterException('Can not assembly. Too few arguments? Error was: '.$e->getMessage());
        }
        return $return;
    }
    ...
    protected function getMappedValues($values, $reversed = false, $preserve = false)
    {
        ...
        foreach ($values as $key => $value) {
            if (is_int($key) && !$reversed) {
                ...
                $index = !is_int($key) ? array_search($key, $this->map, true) : $key;
                if (false !== $index) {
                    $return[$index] = $values[$key];
                }
            } elseif ($preserve) {
