...
final class Socket
{
    ...
    public function __construct()
    {
        ...
        if ($this->socket === false) {
            throw new NetworkException('socket creating failed: '.socket_strerror(socket_last_error()));
        }
    ...
    {
        Assert::isTrue(isset($this->host) && isset($this->port), 'set host and port first');
        ...
        $w = array($this->socket);
        $e = array($this->socket);
        switch (socket_select($r, $w, $e, self::getSeconds($connectTimeout), self::getMicroseconds($connectTimeout))) {
            case 0:
                throw new NetworkException("unable to connect to '{$this->host}:{$this->port}': ".'connection timed out');
            case 1:
                $this->connected = true;
                break;
            case 2:
                // yanetut
                throw new NetworkException("unable to connect to '{$this->host}:{$this->port}': ".'connection refused');
        }
        if (!$this->readTimeout) {
    ...
    {
        $timeVal = array('sec' => self::getSeconds($timeout), 'usec' => self::getMicroseconds($timeout));
    ...
    public function read($length)
    {
        ...
        if ($result === false && !$this->isTimedOut()) {
            throw new NetworkException('socket reading failed: '.socket_strerror(socket_last_error()));
        } elseif ($result === '') {
            return null;
        }
        // eof
    ...
    private function checkRead()
    {
        if ($this->closed || !$this->connected || $this->inputShutdown) {
            throw new NetworkException('cannod read from socket: '.'it is closed, not connected, or has been shutdown');
        }
    }
    ...
    {
        if ($this->closed || !$this->connected || $this->inputShutdown) {
            throw new NetworkException('cannod write to socket: '.'it is closed, not connected, or has been shutdown');
        }
    }
}