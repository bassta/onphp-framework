...
class CustomizableDaoSynchronizer
{
    ...
    {
        if ($masterObject && call_user_func(array($this->slave->getObjectName(), 'create')) instanceof SynchronizableObject) {
            $masterObject = call_user_func(array($this->slave->getObjectName(), 'createFromMasterObject'), $masterObject);
        }
        return $masterObject;
    }
}