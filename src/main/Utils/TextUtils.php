...
{
    public static function friendlyFileSize($size, $precision = 2, $units = array(null, 'k', 'M', 'G', 'T', 'P'), $spacePunctuation = false)
    {
        if ($size > 0) {
    ...
    {
        $result = preg_replace('/&(?!(#(([0-9]+)|(x[0-9A-F]+))'.'|([a-z][a-z0-9]*));)/i', '&amp;', $text);
        return $result;
    }
