...
abstract class FileArchive
{
    ...
    public function __construct($cmdBinPath = null)
    {
        if ($cmdBinPath !== null) {
            if (!is_executable($cmdBinPath)) {
                throw new WrongStateException('cannot find executable '.$cmdBinPath);
            }
            $this->cmdBinPath = $cmdBinPath;
        }
    }

    public abstract function readFile($fileName);
    ...
    public function open($sourceFile)
    {
        if (!is_readable($sourceFile)) {
            throw new WrongStateException('cannot open file '.$sourceFile);
        }
    ...
    {
        if (!$this->cmdBinPath) {
            throw new WrongStateException('nothing to exec');
        }
        ...
        $output = ob_get_clean();
        if ($exitStatus != 0) {
            throw new ArchiverException($this->cmdBinPath.' failed with error code = '.$exitStatus);
        }
        return $output;
    }
}