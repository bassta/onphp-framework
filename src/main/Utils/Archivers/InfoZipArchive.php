...
final class InfoZipArchive extends FileArchive
{
    ...
    {
        if (!$this->sourceFile) {
            throw new WrongStateException('dude, open an archive first.');
        }
        if ($this->zipArchive) {
            $result = $this->zipArchive->getFromName($fileName);
            if ($result === false) {
                throw new ArchiverException('ZipArchive::getFromName() failed');
            }
            return $result;
        }
        $options = '-c -q'.' '.escapeshellarg($this->sourceFile).' '.escapeshellarg($fileName);
