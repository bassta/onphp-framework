...
class DoctypeDeclaration
{
    ...
    {
        $matches = array();
        if (!preg_match('~^('.self::ID_FIRST_CHAR_MASK.self::ID_CHAR_MASK.'*)'.self::SPACER_MASK.'+(.*)$~s', $string, $matches)) {
            return null;
        }
        $this->rootElement = $matches[1];
        $remainigString    = $matches[2];
        if (preg_match('~^PUBLIC'.self::SPACER_MASK.'+"(.+?)"'.'('.self::SPACER_MASK.'*"(.+)")?$~is', $remainigString, $matches)) {
            $this->public       = true;
            $this->inline       = false;
        ...
        } elseif (preg_match('~^SYSTEM'.self::SPACER_MASK.'+"(.+?)"$~is', $remainigString, $matches)) {
            $this->public       = false;
            $this->inline       = false;
            $this->declarations = null;
            $this->setFpi(null);
    ...
    public function toString()
    {
        if ($this->inline) {
            ...
            return $this->rootElement.' PUBLIC "'.$this->getFpi().'"'.($this->uri ? ' "'.$this->uri.'"' : null);
        } else {
            return $this->rootElement.' SYSTEM "'.$this->getFpi().'"';
        }
    }
}