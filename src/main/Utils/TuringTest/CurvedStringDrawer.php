...
final class CurvedStringDrawer extends TextDrawer
{
    ...
    public function draw($string)
    {
        ...
        $textWidth   = $this->getTextWidth($string) + (strlen($string) - 1) * $this->getSize() / 2;
        if ($turingImage->getWidth() <= $textWidth) {
            return $this->showError();
        }
        $angle     = mt_rand(-CurvedStringDrawer::MAX_ANGLE_CHANGE / 2, CurvedStringDrawer::MAX_ANGLE_CHANGE / 2);
        ...
        for ($size = strlen($string), $i = 0; $i < $size; ++$i) {
            ...
            $y += mt_rand(-$turingImage->getHeight() / 2, $turingImage->getHeight() / 2);
