...
final class OneToManyLinkedLazy extends OneToManyLinkedWorker
{
    ...
    {
        $query = $this->makeSelectQuery()->dropFields()->get($this->container->getChildIdField());
    ...
    public function sync($insert, $update = array(), $delete)
    {
        ...
        if ($delete) {
            // unlink or drop
            $uc->isUnlinkable() ? $db->queryNull($this->makeMassUpdateQuery($delete)) : $db->queryNull(OSQL::delete()
                                                                                                           ->from($dao->getTable())
                                                                                                           ->where(Expression::in($uc->getChildIdField(), $delete)));
    ...
    {
        $uc = $this->container;
        return OSQL::update($uc->getDao()->getTable())->set($uc->getParentIdField(), null)
                   ->where(Expression::in($uc->getChildIdField(), $ids));
    }
}