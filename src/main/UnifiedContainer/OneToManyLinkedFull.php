...
final class OneToManyLinkedFull extends OneToManyLinkedWorker
{
    ...
    public function sync($insert, $update = array(), $delete)
    {
        ...
        $dao = $uc->getDao();
        if ($delete) {
            DBPool::getByDao($dao)->queryNull(OSQL::delete()->from($dao->getTable())
                                                  ->where(Expression::eq(new DBField($uc->getParentIdField()), $uc->getParentObject()
                                                                                                                  ->getId()))
                                                  ->andWhere(Expression::in($uc->getChildIdField(), ArrayUtils::getIdsArray($delete))));
