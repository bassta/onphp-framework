...
final class YandexRssItemWorker extends Singleton implements FeedItemWorker
{
    ...
    public function makeItems(SimpleXMLElement $xmlFeed)
    {
        ...
        if (isset($xmlFeed->channel->item)) {
            foreach ($xmlFeed->channel->item as $item) {
                $feedItem = YandexRssFeedItem::create((string)$item->title)->setContent(FeedItemContent::create()
                                                                                                       ->setBody((string)$item->description))
                                             ->setPublished(Timestamp::create(strtotime((string)$item->pubDate)))
                                             ->setFullText((string)$fullTextList[$i++])->setLink((string)$item->link);
                if (isset($item->guid)) {
    ...
    {
        return '<item>'.($item->getPublished() ? '<pubDate>'.date('r', $item->getPublished()
                                                                            ->toStamp()).'</pubDate>' : null).($item->getId() ? '<guid isPermaLink="false">'.$item->getId().'</guid>' : null).'<title>'.$item->getTitle().'</title>'.($item->getLink() ? '<link>'.str_replace('&', '&amp;', $item->getLink()).'</link>' : null).($item->getSummary() ? '<description>'.$item->getSummary().'</description>' : null).($item->getFullText() ? '<yandex:full-text>'.$item->getFullText().'</yandex:full-text>' : null).($item->getCategory() ? '<category>'.$item->getCategory().'</category>' : null).'</item>';
    }
}