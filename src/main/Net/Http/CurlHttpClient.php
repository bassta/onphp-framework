...
final class CurlHttpClient implements HttpClient
{
    ...
    {
        $response = CurlHttpResponse::create()->setMaxFileSize($this->maxFileSize);
    ...
    protected function makeHandle(HttpRequest $request, CurlHttpResponse $response)
    {
        ...
        $options = array(CURLOPT_WRITEFUNCTION => array($response, 'writeBody'), CURLOPT_HEADERFUNCTION => array($response, 'writeHeader'), CURLOPT_URL => $request->getUrl()
                                                                                                                                                                   ->toString(), CURLOPT_USERAGENT => 'onPHP::'.__CLASS__);
        if ($this->noBody !== null) {
            ...
            case HttpMethod::GET:
                ...
                if ($request->getGet()) {
                    $options[CURLOPT_URL] .= ($request->getUrl()
                                                      ->getQuery() ? '&' : '?').$this->argumentsToString($request->getGet());
                }
                break;
    ...
    {
        Assert::isTrue(is_readable($value) && is_file($value), 'couldn\'t access to file with path: '.$value);
        return '@'.$value;
    }
}