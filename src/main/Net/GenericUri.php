...
class GenericUri implements Stringable
{
     ...
     * @return GenericUri
     **/
    public static final function parse($uri, $guessClass = false)
    {
        static $schemePattern = '([^:/?#]+):';
        static $authorityPattern = '(//([^/?#]*))';
        static $restPattern = '([^?#]*)(\\?([^#]*))?(#(.*))?';
        $matches = array();
        if ($guessClass && ($knownSubSchemes = static::getKnownSubSchemes()) && preg_match("~^{$schemePattern}~", $uri, $matches) && isset($knownSubSchemes[strtolower($matches[1])])) {
    ...
    public final function transform(GenericUri $reference, $strict = true)
    {
        ...
        if ($reference->getScheme() !== ($strict ? null : $this->getScheme())) {
            ...
            $result = new $class();
            $result->setScheme($reference->getScheme())->setUserInfo($reference->getUserInfo())
                   ->setHost($reference->getHost())->setPort($reference->getPort())
                   ->setPath(self::removeDotSegments($reference->getPath()))->setQuery($reference->getQuery());
        } else {
            $result = new $this();
            $result->setScheme($this->getScheme());
            if ($reference->getAuthority() !== null) {
                $result->setUserInfo($reference->getUserInfo())->setHost($reference->getHost())
                       ->setPort($reference->getPort())->setPath(self::removeDotSegments($reference->getPath()))
                ...
                if (!$path) {
                    $result->setPath($this->getPath())
                           ->setQuery($reference->getQuery() !== null ? $reference->getQuery() : $this->getQuery());
                } else {
    ...
    public function setAuthority($authority)
    {
        ...
        if (!preg_match($authorityPattern, $authority, $authorityMatches)) {
            throw new WrongArgumentException('not well-formed authority part');
        }
    ...
    {
        return $this->isValidScheme() && $this->isValidUserInfo() && $this->isValidHost() && $this->isValidPort() && $this->isValidPath() && $this->isValidQuery() && $this->isValidFragment();
    }

    public function isValidScheme()
    {
        // empty string is NOT valid
        return $this->scheme === null || preg_match('~^[a-z][-+.a-z0-9]*$~i', $this->scheme) == 1;
    }
    ...
    public function isValidHost()
    {
        ...
        $decOctet = '(\\d)|'.'([1-9]\\d)|'.'(1\\d\\d)|'.'(2[0-4]\\d)|'.'(25[0-5])';
        // 250-255
        ...
        $ipVFutureAddress = "v{$hexdig}+\\.[{$unreserved}{$subDelims}:]+";
        if (preg_match("/^\\[(({$ipV6Address})|({$ipVFutureAddress}))\\]\$/ix", $this->host)) {
            return true;
        }
    ...
    public function isValidPath()
    {
        ...
        if (!preg_match("/^({$charPattern}+)?"."(\\/{$charPattern}*)*\$/i", $this->path)) {
            return false;
        }
    ...
    protected function isValidHostName()
    {
        ...
        return preg_match("/^{$charPattern}*\$/i", $this->host) == 1;
    }

    protected function charPattern($extraChars = null, $pctEncodedPattern = true)
    {
        $unreserved = self::CHARS_UNRESERVED;
        $subDelims  = self::CHARS_SUBDELIMS;
    ...
    public function normalize()
    {
        ...
        // 2. percent-encoded
        $this->setHost($this->normalizePercentEncoded($this->getHost(), $this->hostNameCharPattern(false)))
             ->setUserInfo($this->normalizePercentEncoded($this->getUserInfo(), $this->userInfoCharPattern(false)))
             ->setPath(self::removeDotSegments($this->normalizePercentEncoded($this->getPath(), '\\/'.$this->segmentCharPattern(false))))
             ->setQuery($this->normalizePercentEncoded($this->getQuery(), $this->fragmentOrQueryCharPattern(false)))
             ->setFragment($this->normalizePercentEncoded($this->getFragment(), $this->fragmentOrQueryCharPattern(false)));
        // 3. and case again
    ...
    private function normalizePercentEncoded($string, $unreservedPartChars)
    {
        if ($string === null) {
            return null;
        }
        $result = preg_replace_callback('/(('.self::PATTERN_PCTENCODED.')|(.))/sui', array(PercentEncodingNormalizator::create()
                                                                                                                      ->setUnreservedPartChars($unreservedPartChars), 'normalize'), $string);
        return $result;
    }
}
