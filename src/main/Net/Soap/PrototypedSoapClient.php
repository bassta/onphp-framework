...
abstract class PrototypedSoapClient
{
    ...
    protected function call($method, DTOMessage $request, $resultClass)
    {
        ...
        if (defined('__LOCAL_DEBUG__') && !defined('SIMPLE_TEST')) {
            ...
            Assert::isTrue(!$form->getErrors() && $request->entityProto()
                                                          ->validate($request, $form), Assert::dumpArgument($request));
        }
        try {
            try {
    ...
    {
        return $this->getXmlCdata($this->getSoapClient()->__getLastRequest());
    }

    protected function getLastResponseCdata()
    {
        return $this->getXmlCdata($this->getSoapClient()->__getLastResponse());
    }
