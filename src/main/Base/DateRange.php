...
class DateRange implements Stringable, SingleRange
{
    ...
    public function setStart($start)
    {
        ...
        if ($this->end && $this->end->toStamp() < $start->toStamp()) {
            throw new WrongArgumentException('start must be lower than end');
        }
        $this->start         = $start;
     ...
     * @return DateRange
     **/
    public function safeSetStart($start)
    {
        if (!$this->getEnd() || Timestamp::compare($start, $this->getEnd()) < 0) {
            $this->setStart($start);
        } elseif ($this->getEnd()) {
    ...
    public function setEnd($end)
    {
        ...
        if ($this->start && $this->start->toStamp() > $end->toStamp()) {
            throw new WrongArgumentException('end must be higher than start');
        }
        $this->end         = $end;
    ...
    public function toDateString($internalDelimiter = '-', $dateDelimiter = ' - ')
    {
        if ($this->start && $this->end) {
            return "{$this->start->toDate($internalDelimiter)}".$dateDelimiter."{$this->end->toDate($internalDelimiter)}";
        } elseif ($this->start) {
    ...
    {
        if ($this->start && $this->end) {
            return $this->start->toString().$delimiter.$this->end->toString();
        } elseif ($this->start) {
    ...
    public function overlaps(DateRange $range)
    {
        ...
        return $min && $max && ($left && $right && ($left <= $min && $min <= $right || $min <= $left && $left <= $max) || !$left && $min <= $right || !$right && $left <= $max) || $min && !$max && (!$right || $right && $min <= $right) || !$min && $max && (!$left || $left && $left <= $max);
    }
    ...
    public function contains($date)
    {
        ...
        $probe = $date->toStamp();
        if (!$start && !$end || !$start && $end >= $probe || !$end && $start <= $probe || $start <= $probe && $end >= $probe) {
            return true;
        }
        return false;
    }
    ...
    public function isNeighbour(DateRange $range)
    {
        ...
        if ($this->overlaps($range) || $this->start->spawn('-1 day')
                                                   ->getDayStartStamp() == $range->end->getDayStartStamp() || $this->end->spawn('+1 day')
                                                                                                                        ->getDayStartStamp() == $range->start->getDayStartStamp()
        ) {
            return true;
        }
        return false;
    ...
    public function enlarge(DateRange $range)
    {
        if (!$range->start) {
            $this->start = null;
        } elseif ($this->start && $this->start->toStamp() > $range->start->toStamp()) {
            $this->start = clone $range->start;
        }
        if (!$range->end) {
            $this->end = null;
        } elseif ($this->end && $this->end->toStamp() < $range->end->toStamp()) {
