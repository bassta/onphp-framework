...
abstract class AbstractProtoClass extends Singleton
{
    ...
    protected abstract function makePropertyList();
    ...
    public final function getPropertyList()
    {
        static $lists = array();
    ...
    public final function getExpandedPropertyList($prefix = null)
    {
        static $lists = array();
        $className = get_class($this);
        if (!isset($lists[$className])) {
            foreach ($this->makePropertyList() as $property) {
                if ($property instanceof InnerMetaProperty) {
                    $lists[$className] = array_merge($lists[$className], $property->getProto()
                                                                                  ->getExpandedPropertyList($property->getName().':'));
                } else {
                    $lists[$className][$prefix.$property->getName()] = $property;
                }
            }
        }
        return $lists[$className];
    ...
    public function getPropertyByName($name)
    {
        ...
        throw new MissingElementException(get_class($this).": unknown property requested by name '{$name}'");
    }
    ...
    public function importPrimitive($path, Form $form, BasePrimitive $prm, $object, $ignoreNull = true)
    {
        if (strpos($path, ':') !== false) {
            return $this->forwardPrimitive($path, $form, $prm, $object, $ignoreNull);
        } else {
            $property = $this->getPropertyByName($path);
            $getter   = $property->getGetter();
            if (!$property->isFormless() && $property->getFetchStrategyId() == FetchStrategy::LAZY && !$object->{$getter.'Id'}()) {
                return $object;
            }
            $value = $object->{$getter}();
    ...
    public function exportPrimitive($path, BasePrimitive $prm, $object, $ignoreNull = true)
    {
        if (strpos($path, ':') !== false) {
            return $this->forwardPrimitive($path, null, $prm, $object, $ignoreNull);
        } else {
            $property = $this->getPropertyByName($path);
            ...
            if (!$ignoreNull || $value !== null) {
                ...
                if ($value === null && method_exists($object, $dropper) && (!$property->getRelationId() || $property->getRelationId() == MetaRelation::ONE_TO_ONE)) {
                    $object->{$dropper}();
                    return $object;
    ...
    private static function fetchEncapsulants(Prototyped $object)
    {
        ...
        foreach ($proto->getPropertyList() as $property) {
            if ($property->getRelationId() == MetaRelation::ONE_TO_ONE && $property->getFetchStrategyId() != FetchStrategy::LAZY) {
                ...
                } elseif ($proto->depth && method_exists($property->getClassName(), 'dao')) {
    ...
    private function forwardPrimitive($path, Form $form = null, BasePrimitive $prm, $object, $ignoreNull = true)
    {
        ...
        $getter = $property->getGetter();
        if ($form) {
            return $property->getProto()->importPrimitive($path, $form, $prm, $object->{$getter}(), $ignoreNull);
        } else {
            return $property->getProto()->exportPrimitive($path, $prm, $object->{$getter}(), $ignoreNull);
        }
    }
