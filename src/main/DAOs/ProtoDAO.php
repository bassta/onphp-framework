...
abstract class ProtoDAO extends GenericDAO
{
        ...
        if ($property->getRelationId() == MetaRelation::ONE_TO_MANY || $property->getRelationId() == MetaRelation::MANY_TO_MANY) {
            ...
            if ($property->getRelationId() == MetaRelation::MANY_TO_MANY) {
                ...
                if (!$query->hasJoinedTable($helperAlias)) {
                    $logic = Expression::eq(DBField::create($this->getIdName(), $table), DBField::create($self->{$getter}()
                                                                                                              ->getParentIdField(), $helperAlias));
                    if ($property->isRequired()) {
                ...
                $logic = Expression::eq(DBField::create($propertyDao->getIdName(), $alias), DBField::create($self->{$getter}()
                                                                                                                 ->getChildIdField(), $helperAlias));
            } else {
                $logic = Expression::eq(DBField::create($self->{$getter}()
                                                             ->getParentIdField(), $alias), DBField::create($this->getIdName(), $table));
            }
    ...
    public function guessAtom($atom, JoinCapableQuery $query, $table = null, $parentRequired = true, $prefix = null)
    {
        if ($table === null) {
            $table = $this->getTable();
        }
        if (is_string($atom)) {
            if (strpos($atom, '.') !== false) {
                return $this->processPath(call_user_func(array($this->getObjectName(), 'proto')), $atom, $query, $table, $parentRequired, $prefix);
            } elseif (array_key_exists($atom, $mapping = $this->getMapping())) {
                return new DBField($mapping[$atom], $table);
            } elseif ($query instanceof SelectQuery && $query->hasAliasInside($atom)) {
                return new DBField($atom);
            }
        } elseif ($atom instanceof MappableObject) {
            return $atom->toMapped($this, $query);
        } elseif ($atom instanceof DBValue || $atom instanceof DBField || $atom instanceof DialectString) {
            return $atom;
        }
