...
class ApplicationUrl
{
    ...
    public function addApplicationScope($scope)
    {
        ...
        $this->applicationScope = ArrayUtils::mergeRecursiveUnique($this->applicationScope, $scope);
        return $this;
    }
    ...
    public function addUserScope($userScope)
    {
        ...
        $this->userScope = ArrayUtils::mergeRecursiveUnique($this->userScope, $userScope);
        return $this;
    }
    ...
    public function setPath($path)
    {
        if (!$this->navigationSchema) {
            throw new WrongStateException('charly says always set navigation schema'.' before you go off somewhere');
        }
        $scope = $this->navigationSchema->getScope($path);
        if ($scope === null) {
            throw new WrongArgumentException('404: not found');
        }
    ...
    public function currentHref($additionalScope = array(), $absolute = null)
    {
        return $this->scopeHref(ArrayUtils::mergeRecursiveUnique($this->userScope, $additionalScope), $absolute);
    }
    ...
    public function href($url, $absolute = null)
    {
        ...
        if ($this->applicationScope) {
            $result->appendQuery($this->buildQuery($this->applicationScope), $this->getArgSeparator());
        }
        $result->normalize();
        if ($result->getQuery() === '') {
    ...
    {
        $queryParts = explode($this->getArgSeparator(), $this->buildQuery($scope));
        $result     = array();
    ...
    {
        return http_build_query($scope, null, $this->getArgSeparator());
    }
}