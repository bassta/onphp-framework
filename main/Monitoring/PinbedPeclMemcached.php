...
final class PinbedPeclMemcached extends PeclMemcached
{
    ...
    protected function connect()
    {
        if (PinbaClient::isEnabled()) {
            PinbaClient::me()
                       ->timerStart('pecl_memcached_'.$this->host.'_'.$this->port.'_connect', array('pecl_memcached_connect' => $this->host.'_'.$this->port));
        }
        parent::connect();
        if (PinbaClient::isEnabled()) {
    ...
    private function log($methodName)
    {
        if (PinbaClient::isEnabled()) {
            PinbaClient::me()
                       ->timerStart('pecl_memcached_'.$this->host.'_'.$this->port.'_'.$methodName, array('group' => 'cache', 'memcached_server' => $this->host.'::'.$this->port, 'memcached_type' => $methodName));
        }
    }
