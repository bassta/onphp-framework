    ...
    private function subScanDir($baseNs, array $nsList, $dir)
    {
        ...
        if ($paths = glob($dir.'*', GLOB_ONLYDIR)) {
            foreach ($paths as $subDir) {
                ...
                if (mb_strpos($subNs, '.') !== false || mb_strpos($subNs, '_') !== false) {
                    continue;
                }
                $this->subScanDir($baseNs, array_merge($nsList, array($subNs)), $subDir.DIRECTORY_SEPARATOR);
            }
        }
    }
    ...
    private function scanCurrentDir($baseNs, array $nsList, $dir)
    {
        ...
        if ($paths = glob($dir.'*'.$this->classExtension)) {
            foreach ($paths as $path) {
                $classNsList   = array_merge($nsList, array(basename($path, $this->classExtension)));
