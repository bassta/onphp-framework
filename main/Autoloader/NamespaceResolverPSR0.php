...
class NamespaceResolverPSR0 extends NamespaceResolverOnPHP
{
    ...
    protected function searchClass($className, $namespace, $paths)
    {
        ...
        if (empty($namespace) || $requiredNamespace == $namespace || mb_strpos($requiredNamespace, $namespace) == 0) {
            $subPath   = preg_replace('~\\\\~', self::DS, trim(mb_substr($requiredNamespace, mb_strlen($namespace)), '\\'));
            $checkPath = ($subPath ? $subPath.self::DS : '').$onlyClassName.$this->getClassExtension();
