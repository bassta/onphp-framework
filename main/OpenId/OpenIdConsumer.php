...
final class OpenIdConsumer
{
    ...
    public function __construct(RandomSource $randomSource, BigNumberFactory $numberFactory, HttpClient $httpClient)
    {
     ...
     **/
    public static function create(RandomSource $randomSource, BigNumberFactory $numberFactory, HttpClient $httpClient)
    {
    ...
    public function associate(HttpUrl $server, OpenIdConsumerAssociationManager $manager)
    {
        ...
        $request      = HttpRequest::create()->setMethod(HttpMethod::post())->setUrl($server)
                                   ...
                                   ->setPostVar('openid.dh_modulus', base64_encode($dhParameters->getModulus()
                                                                                                ->toBinary()))
                                   ->setPostVar('openid.dh_gen', base64_encode($dhParameters->getGen()->toBinary()))
                                   ->setPostVar('openid.dh_consumer_public', base64_encode($keyPair->getPublic()
                                                                                                   ->toBinary()));
        ...
        if (isset($result['session_type']) && $result['session_type'] == 'DH-SHA1' && isset($result['dh_server_public'])) {
            $secret = sha1($keyPair->makeSharedKey($this->numberFactory->makeFromBinary(base64_decode($result['dh_server_public'])))
                                   ->toBinary(), true) ^ base64_decode($result['enc_mac_key']);
        ...
        return $manager->makeAndSave($result['assoc_handle'], $result['assoc_type'], $secret, Timestamp::makeNow()
                                                                                                       ->modify('+ '.$result['expires_in'].' seconds'), $server);
    }
    ...
    private function makeCheckIdRequest(OpenIdCredentials $credentials, HttpUrl $returnTo, $trustRoot = null, $association = null)
    {
        ...
        $model = Model::create()->set('openid.ns', self::NAMESPACE_2_0)
                      ->set('openid.identity', $credentials->getRealId()->toString())
                      ->set('openid.return_to', $returnTo->toString())
                      ->set('openid.claimed_id', $credentials->getRealId()->toString());
        foreach ($this->extensions as $extension) {
            $extension->addParamsToModel($model);
        }
        if ($association) {
            Assert::isTrue($association instanceof OpenIdConsumerAssociation && $association->getServer()
                                                                                            ->toString() == $credentials->getServer()
                                                                                                                        ->toString());
     ...
     * @return ModelAndView
     **/
    public function checkIdImmediate(OpenIdCredentials $credentials, HttpUrl $returnTo, $trustRoot = null, $association = null)
    {
        $mav = $this->makeCheckIdRequest($credentials, $returnTo, $trustRoot, $association);
     ...
     * @return ModelAndView
     **/
    public function checkIdSetup(OpenIdCredentials $credentials, HttpUrl $returnTo, $trustRoot = null, $association = null)
    {
        $mav = $this->makeCheckIdRequest($credentials, $returnTo, $trustRoot, $association);
        $mav->getModel()->set('openid.mode', 'checkid_setup');
        return $mav;
    }
...
';
    ...
    {
        $credentials = new OpenIdCredentials(HttpUrl::create()
                                                    ->parse($parameters['openid.identity']), $this->httpClient);
        ...
        $request->setPostVar('openid.mode', 'check_authentication')
                ->setPostVar('openid.assoc_handle', $parameters['openid.assoc_handle'])
                ->setPostVar('openid.sig', $parameters['openid.sig'])
                ->setPostVar('openid.signed', $parameters['openid.signed']);
