...
final class HttpUrl extends Url
{
    ...
    public function isValidScheme()
    {
        ...
        if ($this->scheme && !in_array(strtolower($this->scheme), array('http', 'https'))) {
            return false;
        }
        return true;
    }
    ...
    {
        if ($this->port && !in_array($this->port, array(80, 443)) && $this->port < 1024) {
            return true;
        }
        return false;
    }
    ...
    protected function isValidHostName()
    {
        ...
        return preg_match("/^({$charPattern}*\\.)?{$topLabelPattern}\$/i", $this->host) == 1;
    }
}