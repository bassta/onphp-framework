...
abstract class PrototypedBuilder
{
    ...
    protected abstract function createEmpty();

    /**
     * @return PrototypedGetter
     **/
    protected abstract function getGetter($object);

    /**
     * @return PrototypedSetter
     **/
    protected abstract function getSetter(&$object);
    ...
    {
        throw new UnimplementedFeatureException('reverse builder is not provided yet');
    }
    ...
    public function upperMake($object, &$result)
    {
        if ($this->proto->baseProto()) {
            $this->cloneBuilder($this->proto->baseProto())->upperMake($object, $result);
        }
    ...
    {
        if ($this->proto->baseProto()) {
            $this->cloneBuilder($this->proto->baseProto())->upperFill($object, $result);
        }
        return $this->fillOwn($object, $result);
    }
    ...
    public function fillOwn($object, &$result)
    {
        ...
        foreach ($this->getFormMapping() as $id => $primitive) {
            ...
            if ($primitive instanceof PrimitiveFormsList) {
                $setter->set($id, $this->cloneInnerBuilder($id)->makeList($value));
            } elseif ($primitive instanceof PrimitiveForm) {
                if ($primitive->isComposite() && ($previousValue = $setter->getGetter()->get($id))) {
                    $this->cloneInnerBuilder($id)->upperFill($value, $previousValue);
                } elseif ($value !== null || $primitive->isRequired()) {
                    $setter->set($id, $this->cloneInnerBuilder($id)->make($value));
                }
            } else {
                $setter->set($id, $value);
            }
