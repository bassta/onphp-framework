...
{
    const UNSIGNED_FLAG = 4096;
    private static $limits = array(2 => array(PrimitiveInteger::SIGNED_SMALL_MIN, PrimitiveInteger::SIGNED_SMALL_MAX), 4098 => array(0, PrimitiveInteger::UNSIGNED_SMALL_MAX), 4 => array(PrimitiveInteger::SIGNED_MIN, PrimitiveInteger::SIGNED_MAX), 4100 => array(0, PrimitiveInteger::UNSIGNED_MAX), 8 => array(PrimitiveInteger::SIGNED_BIG_MIN, PrimitiveInteger::SIGNED_BIG_MAX), 4104 => array(0, null));
    private $name = null;
    ...
    public static function fill(LightMetaProperty $property, $name, $columnName, $type, $className, $size, $required, $generic, $inner, $relationId, $strategyId)
    {
        ...
        if ($size) {
            if ($type == 'integer' || $type == 'identifier' || $type == 'integerIdentifier' || $type == 'enumeration' || $type == 'enum') {
        ...
        $property->identifier = $generic && $required && ($type == 'identifier' || $type == 'integerIdentifier' || $type == 'scalarIdentifier');
        return $property;
    }
    ...
    {
        $prm = call_user_func(array('Primitive', $this->type), $name);
    ...
    public function isFormless()
    {
        ...
        return in_array($this->type, array('enumeration', 'enum'));
    }
}