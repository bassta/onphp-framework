...
final class OqlTokenizer
{
    ...
    private $index = -1;
    private static $masks = array(OqlToken::NEW_LINE => '\\n', OqlToken::STRING => '"[^"\\\\]*(?:\\\\.[^"\\\\]*)*"|\'[^\'\\\\]*(?:\\\\.[^\'\\\\]*)*\'|`[^`\\\\]*(?:\\\\.[^`\\\\]*)*`', OqlToken::NUMBER => '(?:\\b[\\d]+)?\\.?[\\d]+(?:[eE][-+]?[\\d]+)?\\b', OqlToken::BOOLEAN => '\\b(?:true|false)\\b', OqlToken::NULL => '\\bnull\\b', OqlToken::SUBSTITUTION => '\\$[\\d]+', OqlToken::KEYWORD => '\\b(?:as|distinct|from|where|not|and|or|in|like|ilike|similar\\s+to|between|is|group\\s+by|order\\s+by|asc|desc|having|limit|offset)\\b', OqlToken::AGGREGATE_FUNCTION => '\\b(?:sum|avg|min|max|count)\\b', OqlToken::IDENTIFIER => '\\b[a-zA-Z_][a-zA-Z\\d_]*(?:\\.[a-zA-Z_][a-zA-Z\\d_]+)*\\b', OqlToken::PARENTHESES => '[\\(\\)]', OqlToken::PUNCTUATION => ',', OqlToken::COMPARISON_OPERATOR => '\\>\\=|\\<\\=|\\<\\>|\\>|\\<|\\!\\=|\\=', OqlToken::ARITHMETIC_OPERATOR => '\\+|\\-|\\/|\\*');
    ...
    private static function importTokenValue($value, $type)
    {
        switch ($type) {
            case OqlToken::STRING:
                ...
                return mb_ereg_replace('\\\\'.$quote, $quote, mb_substr($value, 1, mb_strlen($value) - 2));
