...
final class AMQPPeclChannel extends AMQPBaseChannel
{
    ...
    public function basicAck($deliveryTag, $multiple = false)
    {
        try {
            ...
            $result = $obj->ack($deliveryTag, $multiple === true ? AMQP_MULTIPLE : self::AMQP_NONE);
        } catch (Exception $e) {
            $this->clearConnection();
            throw new AMQPServerException($e->getMessage(), $e->getCode(), $e);
        }
        $this->checkCommandResult($result, 'Could not ack message');
        return $this;
    }
    ...
    public function basicCancel($consumerTag)
    {
        ...
        $this->checkCommandResult($result, 'Could not cancel queue');
        return $this;
    }
    ...
    public function basicGet($queue, $autoAck = true)
    {
        try {
            $obj     = $this->lookupQueue($queue);
            $message = $obj->get($autoAck === true ? AMQP_AUTOACK : self::AMQP_NONE);
        } catch (Exception $e) {
            $this->clearConnection();
            throw new AMQPServerException($e->getMessage(), $e->getCode(), $e);
        }
    ...
    {
        try {
            $result = $this->getChannelLink()->qos($prefetchSize, $prefetchCount);
        } catch (Exception $e) {
            $this->clearConnection();
            throw new AMQPServerException($e->getMessage(), $e->getCode(), $e);
        }
    ...
    public function exchangeBind($destinationName, $sourceName, $routingKey)
    {
        try {
            ...
            $result = $obj->bind($sourceName, $routingKey);
        } catch (Exception $e) {
            $this->clearConnection();
            throw new AMQPServerException($e->getMessage(), $e->getCode(), $e);
        }
        $this->checkCommandResult($result, 'Could not bind exchange');
        return $this;
    }
    ...
    public function exchangeDeclare($name, AMQPExchangeConfig $conf)
    {
        ...
        if (!$conf->getType() instanceof AMQPExchangeType) {
            throw new WrongArgumentException('AMQP exchange type is not set');
        }
        try {
            $this->exchangeList[$name] = new AMQPExchange($this->getChannelLink());
            $obj                       = $this->exchangeList[$name];
            $obj->setName($name);
            $obj->setType($conf->getType()->getName());
            $obj->setFlags($conf->getBitmask(new AMQPPeclExchangeBitmask()));
        ...
        $this->checkCommandResult($result, 'Could not declare exchange');
        return $this;
    }
    ...
    public function exchangeDelete($name, $ifUnused = false)
    {
        ...
        $this->checkCommandResult($result, 'Could not delete exchange');
    ...
    public function queueBind($name, $exchange, $routingKey)
    {
        ...
        $this->checkCommandResult($result, 'Could not bind queue');
        return $this;
    }
    ...
    public function queueDelete($name)
    {
        ...
        $this->checkCommandResult($result, 'Could not delete queue');
    ...
    public function queuePurge($name)
    {
        ...
        $this->checkCommandResult($result, 'Could not purge queue');
        return $this;
    }
    ...
    public function queueUnbind($name, $exchange, $routingKey)
    {
        ...
        $this->checkCommandResult($result, 'Could not unbind queue');
        return $this;
    }
    ...
    {
        if (!$this->link) {
            $this->link = new AMQPChannel($this->getTransport()->getLink());
        }
        return $this->link;
    }
