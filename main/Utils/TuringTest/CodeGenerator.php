...
{
    private static $similarSymbols = array('0', 'o', '1', 'l');
    private $length = null;
    ...
    {
        $variants = array();
        Assert::isTrue($this->lowerAllowed || $this->upperAllowed || $this->numbersAllowed, 'what exactly should i generate?');
        do {
            ...
            shuffle($variants);
            $symbol = $variants[0];
        } while (!$this->similarAllowed && in_array($symbol, self::$similarSymbols));
        return $symbol;
    }
