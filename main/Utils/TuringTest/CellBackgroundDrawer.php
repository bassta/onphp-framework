...
final class CellBackgroundDrawer extends BackgroundDrawer
{
    ...
    public function draw()
    {
        ...
        while ($x < $width) {
            ...
            imageline($this->getTuringImage()->getImageId(), $x, 0, $x, $this->getTuringImage()->getHeight(), $colorId);
            $x += $this->step;
        }
        ...
        while ($y < $height) {
            $color   = $this->makeColor();
            $colorId = $this->getTuringImage()->getColorIdentifier($color);
            imageline($this->getTuringImage()->getImageId(), 0, $y, $this->getTuringImage()->getWidth(), $y, $colorId);
            $y += $this->step;
        }
        return $this;
    }
