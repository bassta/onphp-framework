...
final class RussianTextUtils extends StaticFactory
{
    ...
    const FEMALE = 1;
    const NEUTRAL = 2;
    private static $orderedSuffixes = array(self::MALE => array('ый', 'ой', 'ий'), self::FEMALE => array('ая', 'ья', null), self::NEUTRAL => array('ое', 'ье', null));
    private static $orderedDigits = array('перв', 'втор', 'трет', 'четвёрт', 'пят', 'шест', 'седьм', 'восьм', 'девят', 'десят');
    private static $bytePrefixes = array(null, 'К', 'М', 'Г', 'Т', 'П');
    private static $lettersMapping = array('а' => 'a', 'б' => 'b', 'в' => 'v', 'г' => 'g', 'д' => 'd', 'е' => 'e', 'ё' => 'jo', 'ж' => 'zh', 'з' => 'z', 'и' => 'i', 'й' => 'jj', 'к' => 'k', 'л' => 'l', 'м' => 'm', 'н' => 'n', 'о' => 'o', 'п' => 'p', 'р' => 'r', 'с' => 's', 'т' => 't', 'у' => 'u', 'ф' => 'f', 'х' => 'kh', 'ц' => 'c', 'ч' => 'ch', 'ш' => 'sh', 'щ' => 'shh', 'ъ' => '\'', 'ы' => 'y', 'ь' => '\'', 'э' => 'eh', 'ю' => 'ju', 'я' => 'ja', 'А' => 'A', 'Б' => 'B', 'В' => 'V', 'Г' => 'G', 'Д' => 'D', 'Е' => 'E', 'Ё' => 'JO', 'Ж' => 'ZH', 'З' => 'Z', 'И' => 'I', 'Й' => 'JJ', 'К' => 'K', 'Л' => 'L', 'М' => 'M', 'Н' => 'N', 'О' => 'O', 'П' => 'P', 'Р' => 'R', 'С' => 'S', 'Т' => 'T', 'У' => 'U', 'Ф' => 'F', 'Х' => 'KH', 'Ц' => 'C', 'Ч' => 'CH', 'Ш' => 'SH', 'Щ' => 'SHH', 'Ъ' => '\'', 'Ы' => 'Y', 'Ь' => '\'', 'Э' => 'EH', 'Ю' => 'JU', 'Я' => 'JA');
    private static $monthInGenitiveCase = array('января', 'февраля', 'марта', 'апреля', 'мая', 'июня', 'июля', 'августа', 'сентября', 'октября', 'ноября', 'декабря');
    ...
    public static function selectCaseForNumber($number, $cases)
    {
        if ($number % 10 == 1 && $number % 100 != 11) {
            return $cases[0];
        } elseif ($number % 10 > 1 && $number % 10 < 5 && ($number % 100 < 10 || $number % 100 > 20)) {
            return $cases[1];
        } else {
            return $cases[2];
        }
    ...
    {
        static $weekDays = array('вс', 'пн', 'вт', 'ср', 'чт', 'пт', 'сб', 'вс');
        static $weekDaysFull = array('Воскресенье', 'Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница', 'Суббота', 'Воскресенье');
        if ($full) {
    ...
    public static function toRussian($sourceString)
    {
        if (!self::$flippedLettersMapping) {
            self::$flippedLettersMapping = array_flip(self::$lettersMapping);
        }
    ...
    {
        static $tables = array('KOI8-R' => array(), 'WINDOWS-1251' => array());
        $table = CyrillicPairs::getTable();
        $score = array('UTF-8' => 0, 'KOI8-R' => 0, 'WINDOWS-1251' => 0);
        foreach (preg_split('~[\\.\\,\\-\\s\\:\\;\\?\\!\'\\"\\(\\)\\d<>]~', $data) as $word) {
            for ($i = 0; $i < strlen($word) - 2; ++$i) {
                foreach (array_keys($score) as $encoding) {
                    ...
                    } elseif (isset($tables[$encoding][$pair])) {
                        $value = $tables[$encoding][$pair];
                    } else {
                        $utf8Pair = mb_convert_encoding($pair, 'UTF-8', $encoding);
        ...
        $koi8Ratio  = $score['KOI8-R'] / ($score['WINDOWS-1251'] + $score['UTF-8'] + 1);
        $winRatio   = $score['WINDOWS-1251'] / ($score['KOI8-R'] + $score['UTF-8'] + 1);
        $utf8Ratio  = $score['UTF-8'] / ($score['KOI8-R'] + $score['WINDOWS-1251'] + 1);
        $minRatio   = 1.5;
        $doubtRatio = 1;
        if ($koi8Ratio < $minRatio && $koi8Ratio > $doubtRatio || $winRatio < $minRatio && $winRatio > $doubtRatio || $utf8Ratio < $minRatio && $utf8Ratio > $doubtRatio) {
