...
class CommonDaoWorker extends BaseDaoWorker
{
    ...
    public function getById($id, $expires = Cache::EXPIRES_MEDIUM)
    {
        if ($expires !== Cache::DO_NOT_CACHE && ($object = $this->getCachedById($id))) {
            ...
            $query = $this->dao->makeSelectHead()
                               ->andWhere(Expression::eq(DBField::create($this->dao->getIdName(), $this->dao->getTable()), $id));
            ...
            if ($object) {
                ...
                throw new ObjectNotFoundException('there is no such object for \''.$this->dao->getObjectName().'\' with query == '.$query->toDialectString(DBPool::me()
                                                                                                                                                                 ->getByDao($this->dao)
                                                                                                                                                                 ->getDialect()));
            }
        }
    ...
    public function getByQuery(SelectQuery $query, $expires = Cache::DO_NOT_CACHE)
    {
        if ($expires !== Cache::DO_NOT_CACHE && ($object = $this->getCachedByQuery($query))) {
            ...
            if ($object) {
                ...
                throw new ObjectNotFoundException('there is no such object for \''.$this->dao->getObjectName().(defined('__LOCAL_DEBUG__') ? '\' with query == '.$query->toDialectString(DBPool::me()
                                                                                                                                                                                               ->getByDao($this->dao)
                                                                                                                                                                                               ->getDialect()) : null));
            }
        }
    ...
    public function getCustom(SelectQuery $query, $expires = Cache::DO_NOT_CACHE)
    {
        ...
        if ($expires !== Cache::DO_NOT_CACHE && ($object = $this->getCachedByQuery($query))) {
            ...
            throw new ObjectNotFoundException('zero'.(defined('__LOCAL_DEBUG__') ? 'for query == '.$query->toDialectString(DBPool::me()
                                                                                                                                 ->getByDao($this->dao)
                                                                                                                                 ->getDialect()) : null));
        }
    }
    ...
    public function getListByQuery(SelectQuery $query, $expires = Cache::DO_NOT_CACHE)
    {
        if ($expires !== Cache::DO_NOT_CACHE && ($list = $this->getCachedByQuery($query))) {
            ...
            throw new ObjectNotFoundException('empty list'.(defined('__LOCAL_DEBUG__') ? ' for such query - '.$query->toDialectString(DBPool::me()
                                                                                                                                            ->getByDao($this->dao)
                                                                                                                                            ->getDialect()) : null));
        }
    }
    ...
    public function getQueryResult(SelectQuery $query, $expires = Cache::DO_NOT_CACHE)
    {
        if ($expires !== Cache::DO_NOT_CACHE && ($list = $this->getCachedByQuery($query))) {
            ...
            $count = clone $query;
            $count = DBPool::getByDao($this->dao)->queryRow($count->dropFields()->dropOrder()->limit(null, null)
                                                                  ->get(SQLFunction::create('COUNT', '*')
                                                                                   ->setAlias('count')));
            return $this->cacheByQuery($query, $list ? QueryResult::create()->setList($list)->setCount($count['count'])
                                                                  ->setQuery($query) : QueryResult::create(), $expires);
        }
    }
    //@}
    /// cachers
