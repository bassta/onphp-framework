...
final class HtmlTokenizer
{
    ...
    {
        $matches = preg_match('/^'.self::ID_FIRST_CHAR_MASK.self::ID_CHAR_MASK.'*$/', $id);
        return $matches > 0;
    }
    ...
    public static function removeWhitespaces(Cdata $cdata)
    {
        ...
        $string = preg_replace('/^'.self::SPACER_MASK.'+/', ' ', $string);
        $string = preg_replace('/'.self::SPACER_MASK.'+$/', ' ', $string);
...
'
    ...
    {
        $this->mark = array($this->char, $this->previousChar, $this->line, $this->linePosition);
        $this->stream->mark();
        return $this;
    }
    ...
    private function reset()
    {
        ...
        list($this->char, $this->previousChar, $this->line, $this->linePosition) = $this->mark;
        $this->stream->reset();
        return $this;
    }
    ...
    private function skipString($string, $skipSpaces = false)
    {
        $this->mark();
        if ($skipSpaces) {
            while ($this->char !== null && self::isSpacerChar($this->char)) {
                $this->getNextChar();
            }
        }
    ...
    private function makeTag()
    {
        ...
        if (!$this->suppressWhitespaces || !$this->tag instanceof Cdata || self::removeWhitespaces($this->tag) !== null) {
    ...
    private function handleState()
    {
        switch ($this->state) {
            case self::INITIAL_STATE:
                if ($this->previousTag instanceof SgmlOpenTag && $this->isInlineTag($this->previousTag->getId())) {
    ...
    private function checkSpecialTagState()
    {
        ...
        $specialStartTags = array('![CDATA[' => self::CDATA_STATE, '!--' => self::COMMENT_STATE);
    ...
    private function outsideTagState()
    {
        ...
        while ($this->char !== null) {
            if ($this->char != '<') {
                ...
                $this->getNextChar();
                if (self::isIdFirstChar($this->char) || $this->char == '?' || $this->char == '!') {
                    $this->dumpBuffer();
    ...
    private function startTagState()
    {
        ...
        while ($this->char !== null) {
            if ($this->char == '>') {
                ...
                $externalTag = $this->tagId[0] == '?' && $this->tagId != '?xml';
                $doctypeTag  = strtoupper($this->tagId) == '!DOCTYPE';
                if ($externalTag) {
                    $this->setupTag(SgmlIgnoredTag::create()->setEndMark('?'));
                } elseif ($doctypeTag) {
    ...
    private function dumpEndTag()
    {
        ...
        $this->tag = SgmlEndTag::create()->setId(self::optionalLowercase($this->tagId, $this->lowercaseTags));
        $this->makeTag();
        return $this;
    }
    ...
    {
        Assert::isNull($this->tag);
        Assert::isTrue($this->tagId === null || $this->char == '>' || self::isSpacerChar($this->char));
        Assert::isNull($this->attrName);
        Assert::isNull($this->attrValue);
        Assert::isNull($this->insideQuote);
        $eatingGarbage = false;
    ...
    private function cdataState()
    {
        ...
        $this->tag = Cdata::create()->setData($content)->setStrict(true);
        $this->makeTag();
    ...
    private function getContentToSubstring($substring, $ignoreCase = false)
    {
        ...
        while ($this->char !== null) {
            ...
            while (self::optionalLowercase($buffer[$maxLength], $ignoreCase) !== $char && $maxLength > 0) {
                $maxLength = $prefixTable[$maxLength];
            }
            ++$i;
            $prefixTable[$i + 1] = self::optionalLowercase($buffer[$maxLength], $ignoreCase) === $char ? $maxLength + 1 : 0;
            if ($i > $substringLength + 1 && $prefixTable[$i + 1] == $substringLength) {
                $this->substringFound = true;
                break;
            }
        }
        if (!$this->substringFound) {
            return substr($buffer, $substringLength + 1);
        } else {
            return substr($buffer, $substringLength + 1, $i - 2 * $substringLength);
        }
    }

    private function getTextualPosition()
    {
        return "line {$this->line}, position {$this->linePosition}".($this->tag && $this->tag->getId() ? ", in tag '{$this->tag->getId()}'" : null);
    }
    ...
    {
        $this->errors[] = "warning at {$this->getTextualPosition()}: {$message}";
        return $this;
    }
    ...
    {
        $this->errors[] = "error at {$this->getTextualPosition()}: {$message}";
        return $this;
    }
}