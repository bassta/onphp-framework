...
class SocketMemcached extends CachePeer
{
     ...
     **/
    public static function create($host = self::DEFAULT_HOST, $port = self::DEFAULT_PORT, $buffer = self::DEFAULT_BUFFER)
    {
...
';
    ...
    {
        $command = $time ? "delete {$index} {$time}\r\n" : "delete {$index}\r\n";
        if (!$this->sendRequest($command)) {
            return false;
        }
        try {
...
'
    ...
    protected function store($method, $index, $value, $expires = Cache::EXPIRES_MINIMUM)
    {
        ...
        } elseif (Assert::checkFloat($value) && (int)$value != (double)$value) {
            $packed = serialize($value);
            $flags |= 1;
        } else {
            $packed = $value;
    ...
    private function sendRequest($command)
    {
        ...
        if ($commandLenght > $this->buffer) {
            ...
            while ($offset < $commandLenght) {
                try {
                    $result = fwrite($this->link, substr($command, $offset, $this->buffer));
                } catch (BaseException $e) {
            ...
            try {
                return fwrite($this->link, $command, $commandLenght) !== false;
            } catch (BaseException $e) {
                return $this->alive = false;
            }
        }
